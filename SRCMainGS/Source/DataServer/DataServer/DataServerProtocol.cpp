...
void DataServerProtocolCore(int index, BYTE head, BYTE* lpMsg, int size) // OK
{
	...
	case 0x41:
		...
		case 0x00:
			...
			break;
		case 0x01:
			GMHolyItem->GDHolyInventorySaveRecv((SDHP_INVENTORY_HOLY_SAVE_RECV*)lpMsg);
			break;
		}
		break;
	case 0x42:
	...
	case 0xF7:
		...
		case 0x00:
			...
			break;
		case 0x01:
			if (size == sizeof(SDHP_CUSTOMNPCQUEST_SAVE_RECV))
			{
				GDCustomNpcQuestSaveRecv((SDHP_CUSTOMNPCQUEST_SAVE_RECV*)lpMsg);
			}
			break;
		case 0x02:
			if (size == sizeof(SDHP_CUSTOMNPCQUESTMONSTERSAVE_RECV))
			{
	...
	case 0xF8:
		...
		case 0x00:
			...
			break;
		case 0x01:
			gGoblinExpansion.GDInventorySaveRecv((SDHP_MIX_GOBBLIN_ITEM_SAVE_RECV*)lpMsg);
			break;
		}
		break;
	...
	for (int n = 0; n < 5; n++)
	{
		...
		else
		{
			...
			for (int i = 0; i < 12; i++)
			{
				...
				if (Inventory[n][0] == 0xFF && (Inventory[n][7] & 0x80) == 0x80 && (Inventory[n][9] & 0xF0) == 0xF0)
				{
...
{
	gQueryManager.BindParameterAsBinary(1, lpMsg->Inventory[0], sizeof(lpMsg->Inventory));
	gQueryManager.ExecQuery("UPDATE Character SET Inventory=?,Power=%d WHERE AccountID='%s' AND Name='%s'", lpMsg->Powers, lpMsg->account, lpMsg->name);
	gQueryManager.Close();
}
...
void GDCustomNpcQuestSaveRecv(SDHP_CUSTOMNPCQUEST_SAVE_RECV* lpMsg) // OK
{
	...
	if (gQueryManager.ExecQuery("SELECT * FROM CustomNpcQuest WHERE Name='%s' and Quest = %d", name, lpMsg->quest) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		gQueryManager.Close();
		gQueryManager.ExecQuery("INSERT INTO CustomNpcQuest (Name,quest,count,MonsterCount) VALUES ('%s',%d,1,99999)", name, lpMsg->quest);
		gQueryManager.Close();
	...
	{
		gQueryManager.Close();
		gQueryManager.ExecQuery("Update CustomNpcQuest SET Count = Count+1,MonsterCount=99999 WHERE Name = '%s' and Quest = %d", name, lpMsg->quest);
		gQueryManager.Close();
	}
...
{
	if (lpMsg == 0 || lpMsg->quest == 0 || lpMsg->monsterqtd > 1000000)
	{
		return;
	}
	...
	{
		gQueryManager.Close();
		gQueryManager.ExecQuery("INSERT INTO CustomNpcQuest (Name,quest,count,monstercount) VALUES ('%s',%d,0,%d)", name, lpMsg->quest, lpMsg->monsterqtd);
		gQueryManager.Close();
	...
	{
		gQueryManager.Close();
		gQueryManager.ExecQuery("Update CustomNpcQuest SET MonsterCount=%d WHERE Name = '%s' and Quest = %d", lpMsg->monsterqtd, name, lpMsg->quest);
		gQueryManager.Close();
	}
