...
void CGMHeadChat::SetBooleanPosition(CHAT* c)
{
	...
	if (gmProtect->LookAndFeel == 5 && pCharacter->GetKind() == KIND_NPC)
	{
		SIZE nameSize = { 0 };
		g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(),
			c->ID, lstrlen(c->ID), &nameSize);
		c->Width = nameSize.cx + 3;
		c->Height = FontHeight;
		return;
	}
...
bool CGMHeadChat::ProjectCharacterAnchor(const CHARACTER* character, int* screenX, int* screenY) const
{
	if (character == NULL || screenX == NULL || screenY == NULL)
		return false;

	const OBJECT* object = &character->Object;
	const int helperType = character->Helper.Type;
	float mountHeight = 0.0f;

	// Keep only actual model geometry in world space. All visual separation
	// between overlay elements is handled in screen space after projection.
	if (helperType == MODEL_HELPER + 4 || object->movementType == Movement::Horse)
		mountHeight = 70.0f;
	else if (helperType == MODEL_HELPER + 37 || object->movementType == Movement::Fenrir)
		mountHeight = 55.0f;
	...
	Vector(object->Position[0], object->Position[1],
		baseHeight + object->BoundingBoxMax[2] + mountHeight, position);
...
void CGMHeadChat::RenderBooleans()
{
	...
	for (int i = 0; i < MAX_CHAT; i++)
	{
		...
		if (ci->IDLifeTime > 0 || ci->LifeTime[0] > 0)
		{
			...
			if (ci->Owner != NULL)
			{
				if (!ProjectCharacterAnchor(ci->Owner, &ScreenX, &ScreenY))
				{
					ci->x = -10000;
					ci->y = -10000;
					continue;
			...
			{
				if (!CameraProjection::WorldToScreen(ci->Position, &ScreenX, &ScreenY))
				{
					ci->x = -10000;
					ci->y = -10000;
					continue;
			...
			if (pCharacter != NULL && pCharacter->GetKind() == KIND_NPC)
			{
				const float nameOffsetY = (g_pNameWindow != NULL)
					? g_pNameWindow->GetNpcNameOffsetY() : 35.f;
				ci->y = ScreenY - (int)(nameOffsetY * g_fScreenRate_y + 0.5f);
			}
			else
			{
				const int modelGap = (int)(10.0f * g_fScreenRate_y + 0.5f);
				ci->y = ScreenY - modelGap - ci->Height;
			}
		}
	}
...
{
	CHARACTER* pCharacter = c->Owner;
	const bool isLook5Npc = (gmProtect->LookAndFeel == 5 && pCharacter->GetKind() == KIND_NPC);
	...
	if (gmProtect->m_RenderCharacterName && (pCharacter->GetKind() == KIND_PLAYER || pCharacter->GetKind() == KIND_MONSTER))
	{
		...
		if (gmProtect->LookAndFeel == 5 && pCharacter->GetKind() == KIND_MONSTER)
		{
			const int characterIndex = gmCharacters->GetCharacterIndex(pCharacter);
			MonsterHealthBar::RenderMonster(RenderPos.x, RenderPos.y - 10,
				c->ID, pCharacter->MonsterIndex, pCharacter->GetMonsterLevel(),
				pCharacter->GetCurrentLife(), pCharacter->GetMaximumLife(),
				characterIndex == SelectedCharacter);

			if (gmProtect->LookAndFeel == 5 && g_pQuestInfoHUD != NULL)
			{
				g_pQuestInfoHUD->RenderMonsterMarker((WORD)pCharacter->MonsterIndex,
					RenderPos.x, RenderPos.y - 10);
			}
		...
		// Look5 monster identity belongs to MonsterHealthBar. Drawing the generic
		// head-chat ID here as well caused the permanent large yellow duplicate.
		if (!(gmProtect->LookAndFeel == 5 && pCharacter->GetKind() == KIND_MONSTER))
		{
	...
	else
	{
		...
		if (!isLook5Npc &&
			c->x <= MouseX && MouseX < (int)(c->x + c->Width * gwinhandle->GetScreenX() / WindowWidth) &&
		...
		if (isLook5Npc)
		{
			// Look5 keeps NPC identity permanently visible; transient NPC speech and
			// client-side flavor descriptions are intentionally not stacked below it.
			g_pRenderText->SetFont(g_hFont);
			return;
		}
