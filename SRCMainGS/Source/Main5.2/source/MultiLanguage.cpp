...
int CMultiLanguage::ConvertCharToWideStr(std::wstring& wstrDest, LPCSTR lpString)
{
	...
	wstrDest.resize(nLenOfWideCharStr);
	const int converted = MultiByteToWideChar(
		iConversionType, 0, lpString, -1, &wstrDest[0], nLenOfWideCharStr);
	if (converted <= 0)
	{
		wstrDest.clear();
		return 0;
	...
	// MultiByteToWideChar includes the terminator when the source length is -1.
	wstrDest.resize(converted - 1);
	return converted - 1;
}
...
int CMultiLanguage::ConvertWideCharToStr(std::string& strDest, LPCWSTR lpwString, int iConversionType)
{
	...
	if (lpwString == NULL || lpwString[0] == L'\0')
		return 0;
	...
	strDest.resize(nLenOfWideCharStr);
	const int converted = WideCharToMultiByte(
		iConversionType, 0, lpwString, -1, &strDest[0], nLenOfWideCharStr, 0, 0);
	if (converted <= 0)
	{
		strDest.clear();
		return 0;
