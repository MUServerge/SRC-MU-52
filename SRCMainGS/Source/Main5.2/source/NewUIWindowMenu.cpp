...
void SEASON3B::CNewUIWindowMenu::RenderFrame()
{
	...
	y = m_Pos.y;
	SEASON3B::RenderImageF(IMAGE_WINDOW_MENU_FRAME_UP, m_Pos.x, y, 112.f, 45.f);
	y += 45.f;
	...
	{
		SEASON3B::RenderImageF(IMAGE_WINDOW_MENU_FRAME_MIDDLE, m_Pos.x, y, 112.f, 15.f);
		y += 15.f;
	}

	SEASON3B::RenderImageF(IMAGE_WINDOW_MENU_FRAME_DOWN, m_Pos.x, y, 112.f, 45.f);

	y = m_Pos.y + 33.f;

	for (int i = 0; i < MENU_MAX_INDEX - 1; ++i)
	{
		SEASON3B::RenderImageF(IMAGE_WINDOW_MENU_LINE, m_Pos.x + 15.f, y, 82.f, 2.f);
		y += 20.f;
	}
}
...
void SEASON3B::CNewUIWindowMenu::RenderArrow()
{
	...
	SEASON3B::RenderImageF(IMAGE_WINDOW_MENU_ARROWL, x, y, 6.f, 9.f);
	x = m_Pos.x + 90.f;
	SEASON3B::RenderImageF(IMAGE_WINDOW_MENU_ARROWR, x, y, 6.f, 9.f);
}
