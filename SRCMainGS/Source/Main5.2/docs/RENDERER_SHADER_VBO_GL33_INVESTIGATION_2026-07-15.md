...
| GL legacy shader | Removed during shader consolidation | `Client\Shaders\shader.vs` / `shader.fs` remain unowned legacy assets | None in the production runtime | The old `RenderVertexBuffer` adapter has no verified caller; its global `SetPerspective` upload was removed | Compatibility built-ins; the old program did not match the queried projection/view/model uniforms | Fixed-function behavior remains the fallback | Dormant architecture removed; active VBO Model ownership is unchanged |
...
| Historical `CShaderGL::SetPerspective` path | Removed | The active VBO Model path uploads the current projection per draw through its scoped program owner |
...
- The historical direct `CShaderGL::SetPerspective`/`run_projection` ownership conflict was removed during shader consolidation; active program changes now route through `CShaderScene`.
