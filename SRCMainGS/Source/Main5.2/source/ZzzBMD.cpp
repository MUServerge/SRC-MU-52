...
#ifdef SHADER_VERSION_TEST
// Context captured by BMD::Transform for the Effect\VBO GPU-skinning draw.
...
bool BMD::NeedsCpuVertexTransform(bool Translate, float _Scale, BMDTransformPolicy policy) const
{
	...
	if (Translate || BoneScale != 1.f || _Scale != 0.f)
		return true;
	...
	for (int i = 0; i < NumMeshs; ++i)
	{
		...
		if (mesh->NumTriangles > 0 && mesh->VAO == 0)
			return true;
	}
...
bool BMD::NeedsCpuNormalTransform(bool needsCpuVertexTransform) const
{
...
void BMD::CreateVertexBuffer(Mesh_t& mesh)
{
	...
	for (int j = 0; j < mesh.NumTriangles; j++)
	{
		...
		for (int p = 0; p < passes; ++p)
		{
			for (int c = 0; c < 3; ++c)
			{
				...
				// Bail out to the legacy path on any malformed index (no partial VBO).
				if (tr->VertexIndex[k] < 0 || tr->VertexIndex[k] >= mesh.NumVertices ||
					tr->NormalIndex[k] < 0 || tr->NormalIndex[k] >= mesh.NumNormals ||
					tr->TexCoordIndex[k] < 0 || tr->TexCoordIndex[k] >= mesh.NumTexCoords)
				{
					mesh.VBO_ElementCount = 0;
					return;
				}
	...
	// 16-bit index buffer: leave oversized meshes on the legacy path (VAO stays 0).
	if (indices.empty() || indices.size() > 65000)
		return;

	mesh.VBO_ElementCount = (GLsizei)indices.size();
...
// GPU-skinned draw of one mesh through the matching Data\Effect\VBO program.
// Uses the static bind-pose VBO built by CreateVertexBuffer plus the per-object
// bone matrices captured by BMD::Transform. Camera matrices are read back from
	...
	ShaderMatrixSnapshot* matrixSnapshot)
{
	...
	g_RenderProfiler.AddCounter(RPC_GPU_SKINNED_MESHES);
	RenderProfilerBindVertexArray(0);

	// Return the context to a state the legacy client-array / immediate-mode path
	// (terrain, effects, non-VBO meshes) expects. In the compatibility profile a
	// left-over enabled generic attrib array (attrib 0 aliases gl_Vertex) or a bound
	// ARRAY_BUFFER silently corrupts glVertexPointer/glColorPointer draws — which was
	// washing out terrain and breaking effects.
