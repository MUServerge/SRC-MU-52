...
void SEASON3B::CNewUIInvenExpansion::SetButtonInfo()
{
	...
	m_BtnPurchase.ChangeButtonImgState(true, IMAGE_STORAGE_BTN_LOCK, false, false, false, 0.f, 0.f, 144.f, 116.f);
...
void SEASON3B::CNewUIInvenExpansion::RenderInvenExpansion()
{
	...
	for (size_t i = 0; i < MAX_INVENT_EXPANSION && i < gmProtect->MaxInventoryExtension; i++)
	{
		...
		RenderImageF(item_add_table, (float)(m_InvBoxPos[i].x - 4), (float)(m_InvBoxPos[i].y - 3), 173.0, 87.0, 0.f, 0.f, 692.f, 348.f);
#else
		RenderImageF(item_add_table, (float)(m_InvBoxPos[i].x - 4), (float)(m_InvBoxPos[i].y - 3), 173.0, 87.0, 0.f, 0.f, 692.f, 348.f);
		...
		SEASON3B::RenderImageF(item_add_marking_non, (float)m_InvBoxPos[i].x, (float)m_InvBoxPos[i].y, 161.0f, 81.0f);
		...
		{
			RenderImageF(i + item_add_marking_no01, (float)(m_InvBoxPos[i].x + 68), (float)(m_InvBoxPos[i].y + 26), 25.f, 28.f, 0.f, 0.f, 100.f, 112.f);
		}
	}
}
