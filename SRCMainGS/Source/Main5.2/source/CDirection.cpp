...
namespace
{
	const float DIRECTION_EPSILON = 0.001f;

	float ClampDirectionValue(float value, float minValue, float maxValue)
	{
		if (value < minValue)
			return minValue;
		if (value > maxValue)
...
bool CDirection::DirectionCameraMove()
{
	if (m_bCameraCheck)
	{
		...
		if (m_fLength <= DIRECTION_EPSILON)
		{
		...
		m_fCount += (float)timefac(m_fCameraSpeed);
...
{
	while (!stl_Monster.empty())
	{
		DeleteCharacter(stl_Monster.front().m_Index + NUMOFMON);
...
float CDirection::CalculateAngle(CHARACTER* c, int x, int y, float Angle)
{
	...
	float dot = ClampDirectionValue(DotProduct(vResult, vTemp2), -1.0f, 1.0f);
	float fAngle = acos(dot) / 3.14159f * 180.0f;
...
bool CDirection::MoveCreatedMonster(int Index, int x, int y, float Angle, int Speed)
{
	...
	if (c == NULL || !c->Object.Live || c->Key != Index + NUMOFMON)
		return false;
