...
// Dedicated texture slots for the Look-5 / MuDream HUD (see _TextureIndex.h).
#define IMG_L5_HELPER_GEAR	(BITMAP_LOOK5_HUD_BEGIN + 0)	// UI_HelperSettingsButton
#define IMG_L5_EXP_BG		(BITMAP_LOOK5_HUD_BEGIN + 1)	// ActionBarsView_I2AC
#define IMG_L5_EXP_FILL		(BITMAP_LOOK5_HUD_BEGIN + 2)	// ActionBarsView_I2B0
...
namespace
{
	const float LOOK5_FRAME_TEXTURE_W = 1224.0f;
	const float LOOK5_FRAME_TEXTURE_H = 180.0f;
	const float LOOK5_FRAME_W = 558.0f;
	const float LOOK5_FRAME_H = 82.0f;
	const float LOOK5_GAUGE_TEXTURE_W = 346.0f;
	const float LOOK5_GAUGE_TEXTURE_H = 11.0f;
	const float LOOK5_GAUGE_W = LOOK5_GAUGE_TEXTURE_W * LOOK5_FRAME_W / LOOK5_FRAME_TEXTURE_W;
	const float LOOK5_GAUGE_H = 8.0f;
	const float LOOK5_BUTTON_W = 72.0f;
	const float LOOK5_BUTTON_H = 24.0f;
	const float LOOK5_BUTTON_TEXTURE_W = 172.0f;
	const float LOOK5_BUTTON_TEXTURE_H = 56.0f;
	const float LOOK5_DREAM_SHOP_X = 500.0f;
	const float LOOK5_MU_HELPER_X = 70.0f;
	const float LOOK5_BUTTON_Y = -30.0f;
	const float LOOK5_GEAR_X = 50.0f;
	const float LOOK5_GEAR_Y = -27.0f;
	const float LOOK5_GEAR_SIZE = 18.0f;
	const float LOOK5_EXP_X = 4.0f;
	const float LOOK5_EXP_Y_BASE = 78.0f;
	const float LOOK5_EXP_Y = 71.0f;
	const float LOOK5_EXP_W = 640.0f;
	const float LOOK5_EXP_H = 6.0f;
	const float LOOK5_SLOT_SELECTED_TEXTURE_SIZE = 52.0f;
	const float LOOK5_SKILL_HOVER_SIZE = 24.0f;
	const float LOOK5_ITEM_HOVER_SIZE = 18.0f;

	void RenderLook5SlotHover(float x, float y, float size)
	{
		glColor4f(1.f, 1.f, 1.f, 1.f);
		SEASON3B::RenderImageF(
			SEASON3B::CNewUISkillList::IMAGE_SLOT_SELECTED,
			x, y, size, size,
			0.f, 0.f, LOOK5_SLOT_SELECTED_TEXTURE_SIZE, LOOK5_SLOT_SELECTED_TEXTURE_SIZE);
	}

	void RenderCenteredTipText(float centerX, float y, const char* text)
	{
		SIZE textSize = { 0, 0 };
		g_pRenderText->SetFont(g_hFont);
		g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), text, lstrlen(text), &textSize);
		const float virtualWidth = (float)textSize.cx / g_fScreenRate_x;
		RenderTipText((int)(centerX - virtualWidth * 0.5f), (int)y, text);
	}

	float ClampRatio(DWORD current, DWORD maximum)
	{
		if (maximum == 0)
			return 0.0f;
	...
	void ConfigureLook5HudButton(SEASON3B::CNewUIButton& button, float x, float y, float atlasY)
	{
		const GLuint image = SEASON3B::CNewUIMainFrameWindow::IMAGE_MENU_BTN_CSHOP;

		button.ChangeButtonImgState(true, image, true, true, false, 0.0f, atlasY, LOOK5_BUTTON_TEXTURE_W, LOOK5_BUTTON_TEXTURE_H);
		button.RegisterButtonState(SEASON3B::BUTTON_STATE_UP, image, 0, LOOK5_BUTTON_TEXTURE_W, atlasY, LOOK5_BUTTON_TEXTURE_W, LOOK5_BUTTON_TEXTURE_H);
		button.RegisterButtonState(SEASON3B::BUTTON_STATE_OVER, image, 1, LOOK5_BUTTON_TEXTURE_W * 2.0f, atlasY, LOOK5_BUTTON_TEXTURE_W, LOOK5_BUTTON_TEXTURE_H);
		button.RegisterButtonState(SEASON3B::BUTTON_STATE_DOWN, image, 2, 0.0f, atlasY, LOOK5_BUTTON_TEXTURE_W, LOOK5_BUTTON_TEXTURE_H);
		button.ChangeButtonInfo(x, y, LOOK5_BUTTON_W, LOOK5_BUTTON_H);
		button.ChangeImgColor(SEASON3B::BUTTON_STATE_UP, RGBA(255, 255, 255, 255));
		button.ChangeImgColor(SEASON3B::BUTTON_STATE_DOWN, RGBA(255, 255, 255, 255));
		button.ChangeFrame(SEASON3B::BUTTON_STATE_UP);
	}

	void SetLook5HudButtonFrame(SEASON3B::CNewUIButton& button)
	{
		button.ChangeButtonState(SEASON3B::BUTTON_STATE_UP, 0);
		button.ChangeButtonState(SEASON3B::BUTTON_STATE_OVER, 1);
		button.ChangeButtonState(SEASON3B::BUTTON_STATE_DOWN, 2);
		button.ChangeFrame(SEASON3B::BUTTON_STATE_UP);
	}

	bool IsLook5GearButtonHit(float posX, float posY)
	{
		return SEASON3B::CheckMouseIn(posX + LOOK5_GEAR_X, posY + LOOK5_GEAR_Y, LOOK5_GEAR_SIZE, LOOK5_GEAR_SIZE);
	}
}
...
void SEASON3B::CNewUIMainFrameWindow::LoadImages()
{
	...
	{
		LoadBitmap("Interface\\HUD\\Look-5\\UI_HUD_Base.tga", IMAGE_MENU_1, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
		...
		// MuDream orb fills (912x1520 = 6 cols x 10 rows of 152x152 frames; the
		// animation in RenderLife64 already uses 152px cells / 6 columns).
		LoadBitmap("Interface\\HUD\\Look-5\\UI_HUD_MANA.tga", IMAGE_GAUGE_BLUE, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
		LoadBitmap("Interface\\HUD\\Look-5\\UI_HUD_LIFE_POISON.tga", IMAGE_GAUGE_GREEN, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
		LoadBitmap("Interface\\HUD\\Look-5\\UI_HUD_LIFE.tga", IMAGE_GAUGE_RED, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
		// MuDream AG/SD gauges are 346x11 (old Menu-AG/SD were 176x32).
		// _Empty = the dark full-length track drawn underneath the filled gauge.
		LoadBitmap("Interface\\HUD\\Look-5\\UI_HUD_AG_Empty.tga", IMAGE_GAUGE_AG_EMPTY, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
		LoadBitmap("Interface\\HUD\\Look-5\\UI_HUD_SD_Empty.tga", IMAGE_GAUGE_SD_EMPTY, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
		LoadBitmap("Interface\\HUD\\Look-5\\UI_HUD_AG.tga", IMAGE_GAUGE_AG, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
		LoadBitmap("Interface\\HUD\\Look-5\\UI_HUD_SD.tga", IMAGE_GAUGE_SD, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
		LoadBitmap("Interface\\HUD\\Look-5\\UI_Buttons.tga", IMAGE_MENU_BTN_CSHOP, GL_LINEAR, GL_CLAMP_TO_EDGE);
		LoadBitmap("Interface\\HUD\\Look-5\\UI_HelperSettingsButton.tga", IMG_L5_HELPER_GEAR, GL_LINEAR, GL_CLAMP_TO_EDGE);
		LoadBitmap("Interface\\HUD\\Look-5\\ActionBarsView_I2AC.tga", IMG_L5_EXP_BG, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
		LoadBitmap("Interface\\HUD\\Look-5\\ActionBarsView_I2B0.tga", IMG_L5_EXP_FILL, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
		LoadBitmap("Interface\\HUD\\Look-5\\ActionBarsView_I2BB.tga", IMG_L5_EXP_DIV, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
	}
	else
	{
		LoadBitmap("Interface\\newui_exbar.jpg", IMAGE_GAUGE_EXBAR, GL_LINEAR);
...
void SEASON3B::CNewUIMainFrameWindow::UnloadImages()
{
	...
	{
		DeleteBitmap(IMAGE_MENU_1);
		DeleteBitmap(IMAGE_GAUGE_AG_EMPTY);
		DeleteBitmap(IMAGE_GAUGE_SD_EMPTY);
		DeleteBitmap(IMG_L5_HELPER_GEAR);
		DeleteBitmap(IMG_L5_EXP_BG);
		DeleteBitmap(IMG_L5_EXP_FILL);
		DeleteBitmap(IMG_L5_EXP_DIV);
	}
	else if (gmProtect->LookAndFeel >= 1 && gmProtect->LookAndFeel <= 4)
	{
...
void SEASON3B::CNewUIMainFrameWindow::SetButtonInfo()
{
	...
	{
		float y_Next = PosY + LOOK5_BUTTON_Y;

		ConfigureLook5HudButton(m_BtnCShop, PosX + LOOK5_DREAM_SHOP_X, y_Next, 0.0f);
		ConfigureLook5HudButton(m_BtnMyInven, PosX + LOOK5_MU_HELPER_X, y_Next, LOOK5_BUTTON_TEXTURE_H);
	}
	else
	{
...
bool SEASON3B::CNewUIMainFrameWindow::Render()
{
	...
	if (gmProtect->LookAndFeel == 5)
	{
		// Draw the AG/SD gauges BEHIND the frame: render them first, then the
		// UI_HUD_Base frame on top so its cut-outs reveal the gauges.
		Render64Energy();
		Render64Shield();
		Render64Frame();

		// ...but draw their value numbers ON TOP of the frame so they stay visible.
		{
			float fx = PositionX_In_The_Mid(0.0);
			float fy = Position_In_The_Down(70.f) + 15.0f;
			glColor4f(0.8f, 0.8f, 0.8f, 1.f);
			SEASON3B::RenderNumber(fx + 200.0f, fy, CharacterAttribute->Shield, 0.85f);     // SD
			SEASON3B::RenderNumber(fx + 453.0f, fy, CharacterAttribute->SkillMana, 0.85f);  // AG
		}

		// Invasion notify icon sits on top of the frame, right above the AG gauge
		// (same rect Render64Energy just used). Clicking it toggles the invasion widget.
		if (gmProtect->InvasionInfoTime)
		{
			float fx = (float)PositionX_In_The_Mid(0) + 322.0f;
			float fy = (float)Position_In_The_Down(60.f) + 3.0f;
			GMInvasionManager->RenderNotifyIcon(fx, fy, LOOK5_GAUGE_W);
		}
	}
	else
		RenderFrame();
	...
	if (gmProtect->LookAndFeel == 5)
	{
		RenderExperience(pos_center() + LOOK5_EXP_X, Position_In_The_Down(LOOK5_EXP_Y_BASE) + LOOK5_EXP_Y, LOOK5_EXP_W, LOOK5_EXP_H);

		RenderButtons();

		// While the helper is running, the MU HELPER button shows the animated
		// RUNNING frames (UI_Buttons rows 2-4 = 9 frames of 172x56, 3 columns).
		if (gmAIController->IsRunning())
		{
			glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
			SEASON3B::RenderFrameAnimation(IMAGE_MENU_BTN_CSHOP, PosX + LOOK5_MU_HELPER_X, PosY + LOOK5_BUTTON_Y, LOOK5_BUTTON_W, LOOK5_BUTTON_H,
				0.0f, LOOK5_BUTTON_TEXTURE_H * 2.0f, LOOK5_BUTTON_TEXTURE_W, LOOK5_BUTTON_TEXTURE_H, 1.0f, 3, 9, LOOK5_BUTTON_TEXTURE_W, LOOK5_BUTTON_TEXTURE_H);
		}
	}
	else
	{
	...
	// FPS + RTT readout, bottom-right corner.
	{
		char strStat[64];
		int iFps = (int)FPS;
		int iPing = gsteady_clock->get_ping_time();

		// Left-aligned, inset ~110px from the right edge so the text stays on-screen.
		// (The previous RenderText + RT3_SORT_RIGHT drew off the right edge.) Mirrors the
		// working runtime_render_fps() readout: RenderFont in pixel space, LEFT sort.
		float rx = (float)GetWindowsX - 110.f;
		float ry = (float)GetWindowsY - 34.f;

		g_pRenderText->SetFont(g_hFontBold);
		g_pRenderText->SetBgColor(0, 0, 0, 0);

		g_pRenderText->SetTextColor(180, 230, 255, 255);
		sprintf(strStat, "FPS: %d", iFps);
		g_pRenderText->RenderFont((int)rx, (int)ry, strStat, 0, 15.0, RT3_SORT_LEFT);

		g_pRenderText->SetTextColor(180, 230, 255, 255);
		sprintf(strStat, "RTT: %d ms", iPing);
		g_pRenderText->RenderFont((int)rx, (int)ry + 15, strStat, 0, 15.0, RT3_SORT_LEFT);
	}
...
void SEASON3B::CNewUIMainFrameWindow::EventOrderWindows(double WindowsX, double WindowsY)
{
	...
	{
		float y_Next = PosY + LOOK5_BUTTON_Y;
		m_BtnCShop.ChangeButtonInfo(PosX + LOOK5_DREAM_SHOP_X, y_Next, LOOK5_BUTTON_W, LOOK5_BUTTON_H);
		m_BtnMyInven.ChangeButtonInfo(PosX + LOOK5_MU_HELPER_X, y_Next, LOOK5_BUTTON_W, LOOK5_BUTTON_H);
	}
	else
	{
		float x_Next = PosX + 489;
...
void SEASON3B::CNewUIMainFrameWindow::RenderFrame()
{
	...
	else
	{
		...
		else
		{
			...
			if (g_pSkillList->IsSkillListUp() == true)
			{
				SEASON3B::RenderImage(IMAGE_MENU_2_1, frameX - 408.0, frameY, 170.0, 40.0);
			}
		}
	}
}
...
int SEASON3B::CNewUIMainFrameWindow::RenderExperience(float x, float y, float width, float height)
{
	...
	if (gmProtect->LookAndFeel == 5)
	{
		const float progress = (fNeedExp > 0.0) ? min(max((float)(fExp / fNeedExp), 0.0f), 1.0f) : 0.0f;
		const float fillInset = width * ((1448.0f - 1408.0f) / 1448.0f) * 0.5f;
		const float fillWidth = width - (fillInset * 2.0f);

		glColor4f(1.f, 1.f, 1.f, 1.f);
		RenderImageF(IMG_L5_EXP_BG, x, y, width, height, 0.0f, 0.0f, 1448.0f, 10.0f);

		if (progress > 0.0f)
		{
			RenderImageF(IMG_L5_EXP_FILL, x + fillInset, y + 4.0f, fillWidth * progress, height, 0.0f, 0.0f, 1408.0f * progress, 8.0f);
		}

		const float segmentWidth = width / 10.0f;
		for (int segment = 1; segment < 10; ++segment)
		{
			RenderImageF(IMG_L5_EXP_DIV, x + (segmentWidth * (float)segment) - 2.0f, y - 3.0f, 4.0f, height + 6.0f, 0.0f, 0.0f, 24.0f, 16.0f);
		}
	}
	else if (m_bExpEffect == true)
	{
		...
		if (m_dwPreExp >= baseExperience)
		{
			...
			{
				float fGapProgress = fProgress - fPreProgress;
	...
	if (gmProtect->LookAndFeel == 5)
	{
		SEASON3B::RenderNumber(x + width - 22.0f, y - 5.0f, iExp, 0.7f);
	}
	else if (gmProtect->checkold_school())
	{
	...
	if (SEASON3B::CheckMouseIn(x, y, width, height) == true)
	{
		...
		RenderCenteredTipText(x + width * 0.5f, y - 59.0f, strTipText);
	}
...
void SEASON3B::CNewUIMainFrameWindow::Render64Frame()
{
	...
	RenderImageF(IMAGE_MENU_1, RenderFrameX + 41.f, RenderFrameY - 11.0f, LOOK5_FRAME_W, LOOK5_FRAME_H, 0.0, 0.0, LOOK5_FRAME_TEXTURE_W, LOOK5_FRAME_TEXTURE_H);
	//--
	RenderImageF(IMAGE_MENU_2, RenderFrameX, RenderFrameY + 69, LOOK5_EXP_W, 9.f, 0.0, 0.0, 1024, 24.0);
	//--
	RenderImageF(IMAGE_MENU_2_1, RenderFrameX + 2.000 - 13.0, RenderFrameY + 52.0, 60.0, 20.0, 120.0, 0.0, -120.0, 39.0);

	RenderImageF(IMAGE_MENU_2_1, RenderFrameX + 578.0 + 13.0, RenderFrameY + 52.0, 60.0, 20.0, 0.000, 0.0, +120.0, 39.0);

	{
		float gx = PosX + LOOK5_GEAR_X;
		float gy = PosY + LOOK5_GEAR_Y;
		float gsu = 0.0f;
		if (IsLook5GearButtonHit(PosX, PosY))
			gsu = SEASON3B::IsRepeat(VK_LBUTTON) ? 70.0f : 35.0f;
		RenderImageF(IMG_L5_HELPER_GEAR, gx, gy, LOOK5_GEAR_SIZE, LOOK5_GEAR_SIZE, gsu, 0.0f, 35.0f, 35.0f);
	}
...
void SEASON3B::CNewUIMainFrameWindow::RenderLife64()
{
	...
	float RenderFrameY = (float)Position_In_The_Down(82.f);
...
void SEASON3B::CNewUIMainFrameWindow::Render64LifeHub()
{
	...
	float RenderFrameX = PositionX_In_The_Mid(0.0);
	float RenderFrameY = Position_In_The_Down(117.f);

	glColor4f(0.8, 0.8, 0.8, 1.f);

	SEASON3B::RenderNumber(RenderFrameX + 137.0, RenderFrameY + 63.0, LifeCur, 0.85);

	SEASON3B::RenderNumber(RenderFrameX + 508.0, RenderFrameY + 63.0, ManaCur, 0.85);

	const float orbY = Position_In_The_Down(82.0f);
	const float lifeOrbX = RenderFrameX + 100.0f;
	const float manaOrbX = RenderFrameX + 470.0f;
	const float orbWidth = 68.0f;
	const float orbHeight = 65.0f;

	if (SEASON3B::CheckMouseIn(lifeOrbX, orbY, orbWidth, orbHeight))
	{
		SEASON3B::RenderNumber(RenderFrameX + 137.0f, RenderFrameY + 73.0f, LifeMax, 0.85f);
	}

	if (SEASON3B::CheckMouseIn(manaOrbX, orbY, orbWidth, orbHeight))
	{
		SEASON3B::RenderNumber(RenderFrameX + 508.0f, RenderFrameY + 73.0f, ManaMax, 0.85f);
	}
}
...
void SEASON3B::CNewUIMainFrameWindow::Render64Energy()
{
	...
	fEnergy = 1.0f - ClampRatio(SkillManaCur, SkillManaMax);

	float RenderFrameX = (float)PositionX_In_The_Mid(0);
	float RenderFrameY = (float)Position_In_The_Down(60.f) + 3.0;
	double porcento = (1.0 - fEnergy);
	const double gaugeW = LOOK5_GAUGE_W;
	double width = (double)(porcento * gaugeW);

	float height = LOOK5_GAUGE_H;
	float PosX = RenderFrameX + 322.0;

	glColor4f(1.0, 1.0, 1.0, 1.0);

	RenderImageF(IMAGE_GAUGE_AG_EMPTY, PosX, RenderFrameY, (float)gaugeW, height, 0.0, 0.0, LOOK5_GAUGE_TEXTURE_W, LOOK5_GAUGE_TEXTURE_H);
	RenderImageF(IMAGE_GAUGE_AG, PosX, RenderFrameY, width, height, 0.0, 0.0, (porcento * LOOK5_GAUGE_TEXTURE_W), LOOK5_GAUGE_TEXTURE_H);
...
void SEASON3B::CNewUIMainFrameWindow::Render64Shield()
{
	...
	fShield = 1.0f - ClampRatio(ShieldCur, ShieldMax);
	...
	float PosX = RenderFrameX + 162.0;
	float height = LOOK5_GAUGE_H;

	glColor4f(1.0, 1.0, 1.0, 1.0);

	RenderImageF(IMAGE_GAUGE_SD_EMPTY, PosX, RenderFrameY, (float)gaugeW, height, 0.0, 0.0, LOOK5_GAUGE_TEXTURE_W, LOOK5_GAUGE_TEXTURE_H);
	RenderImageF(IMAGE_GAUGE_SD, PosX, RenderFrameY, width, height, 0.0, 0.0, (porcento * LOOK5_GAUGE_TEXTURE_W), LOOK5_GAUGE_TEXTURE_H);

}

bool SEASON3B::CNewUIMainFrameWindow::HandleLook5MuHelperButton()
{
	if (gmAIController->IsRunning())
	{
...
bool SEASON3B::CNewUIMainFrameWindow::HandleLook5HelperSettingsButton()
{
	if (gmProtect->LookAndFeel != 5 || !SEASON3B::IsRelease(VK_LBUTTON) || !IsLook5GearButtonHit(PosX, PosY))
		return false;
...
bool SEASON3B::CNewUIMainFrameWindow::BtnProcess()
{
	...
	{
		if (HandleLook5HelperSettingsButton())
		{
			return true;
		}
...
bool SEASON3B::CNewUIMainFrameWindow::BtnProcess()
{
	...
	else if (g_pNewUIHotKey->CanUpdateKeyEvent() == true)
	{
		...
		if (m_BtnMyInven.UpdateMouseEvent() == true)
		{
			if (gmProtect->LookAndFeel == 5)
			{
				// Look-5: MU HELPER button starts/stops the official helper.
		...
		// Look-5: helper-settings gear (left of MU HELPER) opens the macro window.
		if (gmProtect->LookAndFeel == 5 && SEASON3B::IsRelease(VK_LBUTTON)
			&& SEASON3B::CheckMouseIn(PosX + 25.0f, PosY - 25.0f, 20.0f, 20.0f))
		{
			g_pNewUISystem->Toggle(SEASON3B::INTERFACE_MACRO_OFICIAL);
			PlayBuffer(SOUND_CLICK01);
			if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_MACRO_OFICIAL))
...
void SEASON3B::CNewUIItemHotKey::Init()
{
	...
	{
		SetPosition(HOTKEY_Q, RenderFrameX + 352, RenderFrameY + 443.0, RenderFrameX + 360, RenderFrameY + 435.0);

		SetPosition(HOTKEY_W, RenderFrameX + 376, RenderFrameY + 443.0, RenderFrameX + 382, RenderFrameY + 435.0);

		SetPosition(HOTKEY_E, RenderFrameX + 400, RenderFrameY + 443.0, RenderFrameX + 408, RenderFrameY + 435.0);

		SetPosition(HOTKEY_R, RenderFrameX + 423, RenderFrameY + 443.0, RenderFrameX + 430, RenderFrameY + 435.0);
...
void SEASON3B::CNewUIItemHotKey::RenderSlotHover()
{
	if (gmProtect->LookAndFeel != 5)
		return;
	...
	for (int i = 0; i < rendercount; ++i)
	{
		const float x = m_iHotKey[i].m_ItemX;
		const float y = m_iHotKey[i].m_ItemY;

		if (SEASON3B::CheckMouseIn(x, y, 20.f, 20.f))
		{
			RenderLook5SlotHover(
				x + (20.f - LOOK5_ITEM_HOVER_SIZE) * 0.5f,
				y + (20.f - LOOK5_ITEM_HOVER_SIZE) * 0.5f,
				LOOK5_ITEM_HOVER_SIZE);
		}
	}
}
...
void SEASON3B::CNewUIItemHotKey::RenderItemCount()
{
	...
	for (int i = 0; i < rendercount; ++i)
	{
		...
		if (iCount > 0)
		{
			...
			{
				SEASON3B::RenderNumber(x, y, iCount, 0.75);
			}
			else
			{
...
void SEASON3B::CNewUISkillList::LoadImages()
{
	...
	else if (gmProtect->LookAndFeel == 5)
	{
		...
		LoadBitmap("Interface\\HUD\\Look-5\\newui_non_command.jpg", IMAGE_COMMAND, GL_LINEAR);
	...
	if (gmProtect->LookAndFeel == 5)
	{
		// MuDream selected-skill highlight (65x65) replaces both skillbox images.
...
{
	if (gmProtect->LookAndFeel == 5 && GMInvasionManager->IsHudNotificationHovered())
	{
		m_bRenderSkillInfo = false;
		m_EventState = EVENT_NONE;
		return true;
	...
	else if (gmProtect->LookAndFeel == 5)//ubicacion del selector de skill
	{
		FrameY = GetWindowsY - 72.0;

		x = FrameX + 322.5;
		y = FrameY + 29.0f;
		width = 30.f; height = 27.f;
	}
	else
	{
		x = FrameX + 385.f;
	...
	if (!gmProtect->checkold_school())
	{
		...
		else if (gmProtect->LookAndFeel == 5)//manejador de evento del hotkey skill
		{
			x = FrameX + 172.5;
			y = FrameY + 29.0f;
			width = 30.0 * 5.f; height = 27.0;
		}
		else
		{
			x = FrameX + 222.f;
...
#if MAIN_UPDATE > 303
	if ((gmProtect->LookAndFeel == 3 || gmProtect->LookAndFeel == 4) && Hero->m_pPet != NULL)
	{
		width = 32; height = 38;
	...
	else if ((gmProtect->LookAndFeel == 5) && Hero->m_pPet != NULL)
	{
		width = 40; height = 38;
			...
			else if (gmProtect->LookAndFeel == 5)
			{
				...
				for (i = 0; i < 5; ++i)
				{
					...
					if (!(m_iHotKeySkillType[iIndex] == -1
						|| m_iHotKeySkillType[iIndex] >= AT_PET_COMMAND_DEFAULT && m_iHotKeySkillType[iIndex] < AT_PET_COMMAND_END && Hero->m_pPet == NULL))
					{
						// Selected-skill frame BEHIND the icon, so the icon shows inside it.
						if (Hero->CurrentSkill == m_iHotKeySkillType[iIndex])
						{
							// UI_SelectedSkill (65x65) sized to fit the 30x27 slot, full texture UV.
							SEASON3B::RenderImageF(IMAGE_SKILLBOX_EX7002, x - 4, y - 2, 30.0, 30.0, 0.0, 0.0, 65.0, 65.0);
						}

						// Larger icon so it fills the 30x30 selected-skill frame.
						RenderSkillIcon(m_iHotKeySkillType[iIndex], x - 1, y + 1, 24.0, 24.0, false);

						if (SEASON3B::CheckMouseIn(x - 3.f, y, width, height))
						{
							RenderLook5SlotHover(
								x - 1.f + (24.f - LOOK5_SKILL_HOVER_SIZE) * 0.5f,
								y + 1.f + (24.f - LOOK5_SKILL_HOVER_SIZE) * 0.5f,
								LOOK5_SKILL_HOVER_SIZE);
						}
					}
					// SEASON3B::RenderNumber(x + 11, y + 27, iIndex, 0.80);
				}

				x = FrameX + 324.5;//ubicacion del icono de la skill actual
				y = FrameY + 29.0f;

				// Frame BEHIND, then the skill icon on top so it shows inside the frame.
				SEASON3B::RenderImageF(IMAGE_SKILLBOX_EX7001, x - 4, y - 2, 30.0, 30.0, 0.0, 0.0, 65.0, 65.0);
				// Larger icon so it fills the 30x30 selected-skill frame.
				RenderSkillIcon(Hero->CurrentSkill, x - 1, y + 1, 24.0, 24.0, false);
			}
			else
			{
				...
				for (i = 0; i < 5; ++i)
				{
					...
					if (Hero->CurrentSkill == m_iHotKeySkillType[iIndex])
					{
						SEASON3B::RenderImageF(IMAGE_SKILLBOX_USE, x, y, width, height, 0.f, 0.f, 128.f, 152.f);
					}
			...
			if (bySkillType && (bySkillType < AT_SKILL_STUN || bySkillType > AT_SKILL_REMOVAL_BUFF))
			{
				...
				if (bySkillUseType != SKILL_USE_TYPE_MASTER && bySkillUseType != SKILL_USE_TYPE_MASTERLEVEL)
				{
					...
					if (i == Hero->CurrentSkill)
						...
						SEASON3B::RenderImageF(IMAGE_SKILLBOX, x, y, width, height, 0.f, 0.f, 128.f, 152.f);
	...
	if (!gmProtect->checkold_school())
	{
		...
		if (bySkillNumber > 0)
		{
			if (m_bSkillList == true)
			{
				...
				{
					fOrigX = AddMiddleX() + 327.f;   // centre of the skill list; tune to taste
					y = AddPositionY() + 365;
				}
				else
				{
				...
				for (i = 0; i < MAX_MAGIC; ++i)
				{
					...
					if (iSkillType != 0 && (iSkillType < AT_SKILL_STUN || iSkillType > AT_SKILL_REMOVAL_BUFF))
					{
						...
						{
							// Look-5 has its own skill-box atlas. The 65x65 current-skill frame
							// belongs only to the HUD selector and must not wrap every list item.
							if (i == Hero->CurrentSkill)
								SEASON3B::RenderImageF(IMAGE_SKILLBOX_USE, x, y, width, height, 0.f, 0.f, 128.f, 152.f);
							else
								SEASON3B::RenderImageF(IMAGE_SKILLBOX, x, y, width, height, 0.f, 0.f, 128.f, 152.f);
...
{
	const int yOffset = (gmProtect->LookAndFeel == 5) ? 16 : 10;
	::RenderSkillInfo(m_iRenderSkillInfoPosX + 15, m_iRenderSkillInfoPosY - yOffset, m_iRenderSkillInfoType);
}
		...
		else
		{
			...
			if (gmProtect->checkold_school())
			{
				...
				if (Renderback)
				{
					...
					SEASON3B::RenderImageF(IMAGE_SKILLBOX, x + ((width - nwidth) / 2.f), y + ((height - nhight) / 2.f), nwidth, nhight, 0.f, 0.f, 128.f, 144.f);
				}
			}
			RenderBitmap(iSkillIndex, x, y, width, height, fU, fV, uw, vh, true, true, 0.0);
		}
...
void SEASON3B::CNewUIMainFrameWindow::SetBtnState(int iBtnType, bool bStateDown)
{
	...
	case MAINFRAME_BTN_PARTCHARGE:
	{
		if (!gmProtect->checkold_school())
		{
			if (gmProtect->LookAndFeel == 5)
			{
				SetLook5HudButtonFrame(m_BtnCShop);
			}
			else if (bStateDown)
			{
	...
	case MAINFRAME_BTN_MYINVEN:
	{
		if (gmProtect->LookAndFeel == 5)
		{
			SetLook5HudButtonFrame(m_BtnMyInven);
		}
		else if (bStateDown)
		{
