...
bool SEASON3B::CNewUIButton::Render(bool RendOption)
{
	if (m_ButtonInfo.size() != 0)
	{
		...
		{
			BITMAP_t* pTex = &Bitmaps[m_CurImgIndex];
			float texW = (float)pTex->output_width;
			float stateH = (float)pTex->output_height / (float)m_ButtonInfo.size();
			SEASON3B::RenderImageF(m_CurImgIndex, m_Pos.x, m_Pos.y, m_Size.x, m_Size.y,
				m_CurImgColor, 0.f, m_CurImgState * stateH, texW, stateH);
		}
	}
...
{
	m_ImageSourceWidth = 0.0f;
	m_ImageSourceFrameHeight = 0.0f;
...
void CNewUIRadioButton::ChangeImageSourceSize(float width, float frameHeight)
{
	m_ImageSourceWidth = width;
	m_ImageSourceFrameHeight = frameHeight;
}
...
bool CNewUIRadioButton::Render()
{
	if (m_RadioButtonInfo.size() != 0)
	{
		...
		{
			if (m_ImageSourceWidth > 0.0f && m_ImageSourceFrameHeight > 0.0f)
			{
				RenderImageF(m_CurImgIndex, m_Pos.x, m_Pos.y, m_Size.x, m_Size.y, m_CurImgColor, 0.0f, m_CurImgState * m_ImageSourceFrameHeight, m_ImageSourceWidth, m_ImageSourceFrameHeight);
			}
			else
			{
...
void CNewUIRadioGroupButton::ChangeImageSourceSize(float width, float frameHeight)
{
	for (RadioButtonList::iterator iter = m_RadioList.begin(); iter != m_RadioList.end(); ++iter)
	{
		(*iter)->ChangeImageSourceSize(width, frameHeight);
	}
}
