...
bool CNewUIDevilSquareEnter::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	for (int i = 0; i < MAX_ENTER_GRADE; i++)
	{
		...
		m_BtnEnter[i].ChangeButtonImgState(true, IMAGE_ENTERDS_BASE_WINDOW_BTN_ENTER, true, false, false, 0.f, 0.f, 720.f, 116.f);
...
bool CNewUIDevilSquareEnter::Render()
{
	...
	RenderText();
	//RenderImageF(IMAGE_ENTERDS_BASE_WINDOW_BACK, m_Pos.x, m_Pos.y, float(ENTERDS_BASE_WINDOW_WIDTH), float(ENTERDS_BASE_WINDOW_HEIGHT), 0.f, 0.f, (float)((float(ENTERDS_BASE_WINDOW_WIDTH)) * 4.f), (float)((float(ENTERDS_BASE_WINDOW_HEIGHT)) * 4.f));
