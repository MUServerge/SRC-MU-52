...
void SEASON3B::CNewUIKeyPadButton::Render()
{
	...
	{
		SEASON3B::RenderImageF(BITMAP_INVENTORY + 17, GetPosX(), GetPosY(), GetWidth(), GetHeight());
	}
	else if (GetEventState() == EVENT_BTN_DOWN)
	{
		SEASON3B::RenderImageF(BITMAP_INVENTORY + 18, GetPosX(), GetPosY(), GetWidth(), GetHeight());
	}
	else
	{
...
bool SEASON3B::CSystemMenuMsgBox::Create(float fPriority)
{
	...
	LoadBitmap("Interface\\LegendUI\\UI_ExitMenu_Window.tga", BITMAP_INTERFACE_EXITMENU_WINDOW, GL_LINEAR);
	LoadBitmap("Interface\\LegendUI\\buttons_big.tga", BITMAP_INTERFACE_EXITMENU_BUTTONS, GL_LINEAR);

	x = (SCREEN_WIDTH / 2) - (SYSMENU_WIDTH / 2);
	y = (SCREEN_HEIGHT / 2) - (SYSMENU_HEIGHT / 2);
	width = SYSMENU_WIDTH;
	height = SYSMENU_HEIGHT;
...
{
	DeleteBitmap(BITMAP_INTERFACE_EXITMENU_WINDOW);
	DeleteBitmap(BITMAP_INTERFACE_EXITMENU_BUTTONS);
...
{
	// RenderImageF (not RenderImage) - the skin is 388x498 and must be sampled whole, then scaled down.
	SEASON3B::RenderImageF(BITMAP_INTERFACE_EXITMENU_WINDOW, GetPos().x, GetPos().y, (float)SYSMENU_WIDTH, (float)SYSMENU_HEIGHT);

	g_pRenderText->SetFont(g_hFontBold);
	g_pRenderText->SetBgColor(0);
	g_pRenderText->SetTextColor(255, 204, 102, 255);
	g_pRenderText->RenderFont(GetPos().x, GetPos().y + SYSMENU_TITLE_TOP, "System Menu", SYSMENU_WIDTH, 15, RT3_SORT_CENTER);
}
...
void SEASON3B::CSystemMenuMsgBox::SetButtonInfo()
{
	...
	width = SYSMENU_BTN_WIDTH;
	height = SYSMENU_BTN_HEIGHT;
	x = GetPos().x + ((SYSMENU_WIDTH - SYSMENU_BTN_WIDTH) / 2.f);
	y = GetPos().y + SYSMENU_BTN_TOP;
	m_BtnGameOver.SetInfo(BITMAP_INTERFACE_EXITMENU_BUTTONS, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_CUSTOM, true);
	m_BtnGameOver.SetText(GlobalText[381]);

	y += SYSMENU_BTN_HEIGHT + SYSMENU_BTN_GAP;
	m_BtnChooseServer.SetInfo(BITMAP_INTERFACE_EXITMENU_BUTTONS, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_CUSTOM, true);
	m_BtnChooseServer.SetText(GlobalText[382]);

	y += SYSMENU_BTN_HEIGHT + SYSMENU_BTN_GAP;
	m_BtnChooseCharacter.SetInfo(BITMAP_INTERFACE_EXITMENU_BUTTONS, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_CUSTOM, true);
	m_BtnChooseCharacter.SetText(GlobalText[383]);

	y += SYSMENU_BTN_HEIGHT + SYSMENU_BTN_GAP;
	m_BtnOption.SetInfo(BITMAP_INTERFACE_EXITMENU_BUTTONS, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_CUSTOM, true);
	m_BtnOption.SetText(GlobalText[385]);

	y += SYSMENU_BTN_HEIGHT + SYSMENU_BTN_GAP;
	m_BtnCancel.SetInfo(BITMAP_INTERFACE_EXITMENU_BUTTONS, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_CUSTOM, true);
...
void SEASON3B::CDevilSquareRankMsgBox::RenderFrame()
{
	...
	RenderImageF(CNewUIMessageBoxMng::IMAGE_MSGBOX_BACK, x, y, width, height, 0.f, 0.f, (float)((width) * 4.f), (float)((height) * 4.f));

	x = GetPos().x; y = GetPos().y, width = MSGBOX_WIDTH; height = MSGBOX_TOP_HEIGHT;
	RenderImageF(CNewUIMessageBoxMng::IMAGE_MSGBOX_TOP, x, y, width, height, 0.f, 0.f, (float)((width) * 4.f), (float)((height) * 4.f));

	x = GetPos().x; y += MSGBOX_TOP_HEIGHT; width = MSGBOX_WIDTH; height = MSGBOX_MIDDLE_HEIGHT;
	for (int i = 0; i < MIDDLE_COUNT1; ++i)
	{
		RenderImageF(CNewUIMessageBoxMng::IMAGE_MSGBOX_MIDDLE, x, y, width, height, 0.f, 0.f, (float)((width) * 4.f), (float)((height) * 4.f));
		y += height;
	}

	x = GetPos().x; width = MSGBOX_LINE_WIDTH; height = MSGBOX_LINE_HEIGHT;
	RenderImageF(CNewUIMessageBoxMng::IMAGE_MSGBOX_LINE, x, y, width, height, 0.f, 0.f, (float)((width) * 4.f), (float)((height) * 4.f));
	y += height;
	...
	RenderImageF(CNewUIMessageBoxMng::IMAGE_MSGBOX_BOTTOM, x, y, width, height, 0.f, 0.f, (float)((width) * 4.f), (float)((height) * 4.f));

	x = GetPos().x + 13; y = GetPos().y + 75; width = MSGBOX_SEPARATE_LINE_WIDTH; height = MSGBOX_SEPARATE_LINE_HEIGHT;
	RenderImageF(CNewUIMessageBoxMng::IMAGE_MSGBOX_SEPARATE_LINE, x, y, width, height, 0.f, 0.f, 205.f, 2.f);
