...
bool SEASON3B::CNewUISystem::LoadMainSceneInterface()
{
	...
	m_pNewQuestInfoHUD = new CNewUIQuestInfoHUD;
	if (m_pNewQuestInfoHUD->Create(m_pNewUIMng, 0, 0) == false)	// position is recomputed each frame (top-right, resolution-aware)
		return false;
	...
	if (m_NewUIReconnect->Create(m_pNewUIMng, PositionX_The_Mid(CNewUIReconnect::RECONNECT_WIDTH), PositionY_The_Mid(CNewUIReconnect::RECONNECT_HEIGHT)) == false)
	{
		return false;
	}
...
void SEASON3B::CNewUISystem::UnloadMainSceneInterface()
{
	...
	SAFE_DELETE(m_pNewQuestInfoHUD);
...
void SEASON3B::CNewUISystem::Show(DWORD dwKey)
{
	...
	if (m_pNewUIMng)
	{
		...
		case SEASON3B::INTERFACE_MYQUEST:
			...
			if (GMProtect->LookAndFeelType() == 3 || GMProtect->LookAndFeelType() == 4 || GMProtect->LookAndFeelType() == 5)
			{
...
bool SEASON3B::CNewUISystem::IsImpossibleHideInterface(DWORD dwKey)
{
	if (dwKey == SEASON3B::INTERFACE_MAINFRAME
		...
		|| dwKey == SEASON3B::INTERFACE_QUESTINFO_HUD
		)
	{
		return true;
	}
...
CNewUIQuestInfoHUD* SEASON3B::CNewUISystem::GetUI_NewQuestInfoHUD() const
{
	return m_pNewQuestInfoHUD;
}
