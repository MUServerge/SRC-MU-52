...
bool CNewUIStorageInventory::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	else
	{
		...
		for (int i = BTN_INSERT_ZEN; i < MAX_BTN; ++i)
		{
			m_abtn[i].ChangeButtonImgState(true, IMAGE_STORAGE_BTN_INSERT_ZEN + i, false, false, false, 0.f, 0.f, 144.f, 116.f);
...
void CNewUIStorageInventory::RenderBackImage()
{
	...
	if (gmProtect->checkold_school())
	{
		RenderImageF(IMAGE_STORAGE_MONEY, m_Pos.x + 10, m_Pos.y + 342, 170.f, 46.f, 0.f, 0.f, 680.f, 184.f);
	}
	else
	{
		RenderImageF(IMAGE_STORAGE_MONEY, m_Pos.x + 10, m_Pos.y + 342, 170.f, 46.f, 0.f, 0.f, 680.f, 184.f);
...
void CNewUIStorageInventory::ChangeLockBtnImage()
{
	...
	else
	{
		m_abtn[BTN_LOCK].UnRegisterButtonState();
		if (m_bLock)
			m_abtn[BTN_LOCK].ChangeButtonImgState(true, IMAGE_STORAGE_BTN_LOCK, false, false, false, 0.f, 0.f, 144.f, 116.f);
		else
			m_abtn[BTN_LOCK].ChangeButtonImgState(true, IMAGE_STORAGE_BTN_UNLOCK, false, false, false, 0.f, 0.f, 144.f, 116.f);
	}
#endif // test_main_dev
}
