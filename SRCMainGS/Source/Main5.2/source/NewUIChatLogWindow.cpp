...
bool SEASON3B::CNewUIChatLogWindow::RenderFrame()
{
	...
	else
	{
		...
		RenderImageF(IMAGE_DRAG_BTN, fRenderPosX, fRenderPosY - (float)RESIZING_BTN_HEIGHT, (float)m_WndSize.cx, 10.0f, 0.f, 0.f, 281.f, 10.f);
		DisableAlphaBlend();

		glColor4f(1.0f, 1.0f, 1.0f, 1.0f);

		RenderImageF(IMAGE_SCROLL_TOP, fRenderPosX + m_WndSize.cx - SCROLL_BAR_WIDTH - WND_LEFT_RIGHT_EDGE, fRenderPosY + WND_TOP_BOTTOM_EDGE, 7, 3, 0.f, 0.f, 28.f, 12.f);

		for (int i = 0; i < (int)GetNumberOfShowingLines(); i++)
		{
			RenderImageF(IMAGE_SCROLL_MIDDLE, fRenderPosX + m_WndSize.cx - SCROLL_BAR_WIDTH - WND_LEFT_RIGHT_EDGE, fRenderPosY + WND_TOP_BOTTOM_EDGE + (float)(i * SCROLL_MIDDLE_PART_HEIGHT + SCROLL_TOP_BOTTOM_PART_HEIGHT), 7, 15, 0.f, 0.f, 28.f, 60.f);
		}

		RenderImageF(IMAGE_SCROLL_BOTTOM, fRenderPosX + m_WndSize.cx - SCROLL_BAR_WIDTH - WND_LEFT_RIGHT_EDGE, m_WndPos.y - WND_TOP_BOTTOM_EDGE - SCROLL_TOP_BOTTOM_PART_HEIGHT, 7, 3, 0.f, 0.f, 28.f, 12.f);
		...
		RenderImageF(IMAGE_SCROLLBAR_ON, m_ScrollBtnPos.x, m_ScrollBtnPos.y, SCROLL_BTN_WIDTH, SCROLL_BTN_HEIGHT, 0.f, 0.f, 60.f, 120.f);
		DisableAlphaBlend();
	}
