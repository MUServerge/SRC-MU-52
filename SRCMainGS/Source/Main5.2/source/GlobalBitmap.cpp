...
{
    int pngwidth, pngheight, channels_in_file;

    BYTE* imageData = stbi_load(filename.c_str(), &pngwidth, &pngheight, &channels_in_file, 0);
    ...
    if ((channels_in_file != 3 && channels_in_file != 4) || pngwidth > MAX_WIDTH || pngheight > MAX_HEIGHT)
    {
        stbi_image_free(imageData);
        return false;
    }

    int textureWidth = leftoverSize(pngwidth);
    int textureHeight = leftoverSize(pngheight);
    ...
    pNewBitmap->Width = (float)textureWidth;
    pNewBitmap->Height = (float)textureHeight;
    pNewBitmap->output_width = pngwidth;
    pNewBitmap->output_height = pngheight;
    pNewBitmap->Components = channels_in_file;
    pNewBitmap->Ref = 1;
    filename._Copy_s(pNewBitmap->FileName, MAX_BITMAP_FILE_NAME, MAX_BITMAP_FILE_NAME);
    ...
    if (pngwidth != textureWidth || pngheight != textureHeight)
    {
        int row_size = pngwidth * channels_in_file;
