...
void SEASON3B::window_nomal(float RenderOriginX, float RenderOriginY, int type, float division)
{
	...
	else
	{
		...
		RenderImageF(CNewUIMyInventory::IMAGE_INVENTORY_BACK, RenderFrameX, RenderFrameY, WIN_WINDOW_SIZEX, WIN_WINDOW_SIZEY, 0.f, 0.f, (float)((WIN_WINDOW_SIZEX) * 4.f), (float)((WIN_WINDOW_SIZEY) * 4.f));
		if (type == 0)
			RenderImageF(CNewUIMyInventory::IMAGE_INVENTORY_BACK_TOP, RenderFrameX, RenderFrameY, WIN_WINDOW_SIZEX, RenderSizeY);
		else
			RenderImageF(CNewUIMyInventory::IMAGE_INVENTORY_BACK_TOP2, RenderFrameX, RenderFrameY, WIN_WINDOW_SIZEX, RenderSizeY);

		RenderFrameY += RenderSizeY;
		RenderSizeY = WIN_WINDOW_SIZEY - 109.f;

		RenderImageF(CNewUIMyInventory::IMAGE_INVENTORY_BACK_LEFT, RenderFrameX, RenderFrameY, 21.f, RenderSizeY);
		RenderImageF(CNewUIMyInventory::IMAGE_INVENTORY_BACK_RIGHT, RenderFrameX + WIN_WINDOW_SIZEX - 21.f, RenderFrameY, 21.f, RenderSizeY);

		RenderFrameY += RenderSizeY;
		RenderSizeY = 45.f;
		RenderImageF(CNewUIMyInventory::IMAGE_INVENTORY_BACK_BOTTOM, RenderFrameX, RenderFrameY, WIN_WINDOW_SIZEX, RenderSizeY);

		if (division != 0.f)
		{
			RenderImageF(CNewUIMyQuestInfoWindow::IMAGE_MYQUEST_LINE, RenderFrameX + 1, RenderOriginY + division, 188.f, 21.f, 0.f, 0.f, 752.f, 84.f);
		}
	}
#else
	if (division != 0.f)
...
void SEASON3B::window_emergent(CNewUIMessageBoxBase* pMsgBox, int iMiddleFrameCount, int type)
{
	...
	{
		RenderImageF(CNewUIMessageBoxMng::IMAGE_MSGBOX_BACK, iRenderFrameX, iRenderFrameY + 2.f, (iRenderSizeX - MSGBOX_BACK_BLANK_WIDTH), (iRenderSizeY - MSGBOX_BACK_BLANK_HEIGHT), 0.f, 0.f, (float)(((iRenderSizeX - MSGBOX_BACK_BLANK_WIDTH)) * 4.f), (float)(((iRenderSizeY - MSGBOX_BACK_BLANK_HEIGHT)) * 4.f));

		RenderImageF(CNewUIMessageBoxMng::IMAGE_MSGBOX_TOP, iRenderFrameX, iRenderFrameY, iRenderSizeX, MSGBOX_TOP_HEIGHT, 0.f, 0.f, (float)((iRenderSizeX) * 4.f), (float)((MSGBOX_TOP_HEIGHT) * 4.f));

		iRenderFrameY += MSGBOX_TOP_HEIGHT;

		for (int i = 0; i < iMiddleFrameCount; ++i)
		{
			RenderImageF(CNewUIMessageBoxMng::IMAGE_MSGBOX_MIDDLE, iRenderFrameX, iRenderFrameY, iRenderSizeX, MSGBOX_MIDDLE_HEIGHT, 0.f, 0.f, (float)((iRenderSizeX) * 4.f), (float)((MSGBOX_MIDDLE_HEIGHT) * 4.f));
			iRenderFrameY += MSGBOX_MIDDLE_HEIGHT;
		}

		RenderImageF(CNewUIMessageBoxMng::IMAGE_MSGBOX_BOTTOM, iRenderFrameX, iRenderFrameY, iRenderSizeX, MSGBOX_BOTTOM_HEIGHT, 0.f, 0.f, (float)((iRenderSizeX) * 4.f), (float)((MSGBOX_BOTTOM_HEIGHT) * 4.f));
	}
#else
	g_pUIManager->RenderWindowBase(iRenderSizeY, -1, iRenderFrameY);
#endif
