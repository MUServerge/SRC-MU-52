...
bool CNewUIEnterBloodCastle::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	for (int i = 0; i < MAX_ENTER_GRADE; i++)
	{
		...
		m_BtnEnter[i].ChangeButtonImgState(true, IMAGE_ENTERBC_BASE_WINDOW_BTN_ENTER, true, false, false, 0.f, 0.f, 720.f, 116.f);
...
bool CNewUIEnterBloodCastle::Render()
{
	...
	// Base Window
	RenderFrame();
	//RenderImageF(IMAGE_ENTERBC_BASE_WINDOW_BACK, m_Pos.x, m_Pos.y, float(ENTERBC_BASE_WINDOW_WIDTH), float(ENTERBC_BASE_WINDOW_HEIGHT), 0.f, 0.f, (float)((float(ENTERBC_BASE_WINDOW_WIDTH)) * 4.f), (float)((float(ENTERBC_BASE_WINDOW_HEIGHT)) * 4.f));
