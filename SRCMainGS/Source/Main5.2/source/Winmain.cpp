...
int  m_MusicOnOff;
int  m_Resolution;
// int targetIndexFPS; // removed — FPS locked to 60 in steady_clock
int	m_nColorDepth;
...
namespace
{
	bool IsOpenGLVersionAtLeast(int major, int minor, int requiredMajor, int requiredMinor)
	{
		return major > requiredMajor || (major == requiredMajor && minor >= requiredMinor);
	}

	bool ParseOpenGLVersion(const char* version, int& major, int& minor)
	{
		major = 0;
		minor = 0;
		if (version == NULL)
	...
	bool IsValidWGLProcAddress(PROC proc)
	{
	...
	const char* PixelFormatAcceleration(const PIXELFORMATDESCRIPTOR& pfd)
	{
		if ((pfd.dwFlags & PFD_GENERIC_FORMAT) == 0)
			return "ICD/full acceleration";
		if ((pfd.dwFlags & PFD_GENERIC_ACCELERATED) != 0)
			return "MCD/generic acceleration";
		return "generic software";
	}

	void WritePixelFormatDiagnostics(HDC hDC, int pixelFormat, const PIXELFORMATDESCRIPTOR& pfd, bool glewReady)
	{
		g_ErrorReport.Write("<Renderer pixel-format diagnostics>\r\n");
		g_ErrorReport.Write("Selected format\t: %d\r\n", pixelFormat);
		g_ErrorReport.Write("Color RGBA bits\t: %u (%u/%u/%u/%u)\r\n", pfd.cColorBits,
			pfd.cRedBits, pfd.cGreenBits, pfd.cBlueBits, pfd.cAlphaBits);
		g_ErrorReport.Write("Depth/stencil bits\t: %u/%u\r\n", pfd.cDepthBits, pfd.cStencilBits);
		g_ErrorReport.Write("Double buffer\t\t: %s\r\n", (pfd.dwFlags & PFD_DOUBLEBUFFER) ? "yes" : "no");
		g_ErrorReport.Write("Stereo\t\t\t: %s\r\n", (pfd.dwFlags & PFD_STEREO) ? "yes" : "no");
		g_ErrorReport.Write("Acceleration\t\t: %s\r\n", PixelFormatAcceleration(pfd));

		if (!glewReady)
		{
			g_ErrorReport.Write("Sample buffers/samples\t: unavailable (GLEW initialization failed)\r\n");
			return;
		}

		PROC proc = wglGetProcAddress("wglGetPixelFormatAttribivARB");
		if (!IsValidWGLProcAddress(proc))
		{
			g_ErrorReport.Write("Sample buffers/samples\t: unavailable (WGL pixel-format query absent)\r\n");
			return;
		}

		PFNWGLGETPIXELFORMATATTRIBIVARBPROC getPixelFormatAttributes =
			(PFNWGLGETPIXELFORMATATTRIBIVARBPROC)proc;
		const int attributes[2] = { WGL_SAMPLE_BUFFERS_ARB, WGL_SAMPLES_ARB };
		int values[2] = { 0, 0 };
		if (!getPixelFormatAttributes(hDC, pixelFormat, 0, 2, attributes, values))
		{
			g_ErrorReport.Write("Sample buffers/samples\t: unavailable (WGL query failed)\r\n");
			return;
		}

		g_ErrorReport.Write("Sample buffers/samples\t: %d/%d\r\n", values[0], values[1]);
	}

	void WriteEntryPointDiagnostic(const char* name, bool available)
	{
		g_ErrorReport.Write("  %-30s: %s\r\n", name, available ? "available" : "MISSING");
	}
	...
	void WriteOpenGLCapabilityDiagnostics(GLenum glewResult)
	{
		...
		int major = 0;
		int minor = 0;
		const bool versionParsed = ParseOpenGLVersion(version, major, minor);
		g_ErrorReport.Write("Parsed GL version\t: %s", versionParsed ? "" : "unavailable");
		if (versionParsed)
			g_ErrorReport.Write("%d.%d", major, minor);
		g_ErrorReport.Write("\r\n");
		...
		if (versionParsed && IsOpenGLVersionAtLeast(major, minor, 3, 0))
		{
			...
			g_ErrorReport.Write("Context flags\t\t: 0x%X\r\n", contextFlags);
		}
		else
			g_ErrorReport.Write("Context flags\t\t: unavailable (requires GL 3.0)\r\n");

		if (versionParsed && IsOpenGLVersionAtLeast(major, minor, 3, 2))
		{
			GLint profileMask = 0;
			glGetIntegerv(GL_CONTEXT_PROFILE_MASK, &profileMask);
			const char* profile = (profileMask & GL_CONTEXT_CORE_PROFILE_BIT) ? "core" :
				((profileMask & GL_CONTEXT_COMPATIBILITY_PROFILE_BIT) ? "compatibility" : "unspecified");
			g_ErrorReport.Write("Context profile\t\t: %s (0x%X)\r\n", profile, profileMask);
		}
		else
			g_ErrorReport.Write("Context profile\t\t: unavailable (requires GL 3.2)\r\n");

		GLint value = 0;
		glGetIntegerv(GL_MAX_TEXTURE_SIZE, &value);
		g_ErrorReport.Write("GL_MAX_TEXTURE_SIZE\t: %d\r\n", value);

		if (shaderVersion)
		{
			glGetIntegerv(GL_MAX_VERTEX_ATTRIBS, &value);
			g_ErrorReport.Write("GL_MAX_VERTEX_ATTRIBS\t: %d\r\n", value);
			glGetIntegerv(GL_MAX_VERTEX_UNIFORM_COMPONENTS, &value);
			g_ErrorReport.Write("Vertex uniform components: %d\r\n", value);
		}
		else
		{
			g_ErrorReport.Write("GL_MAX_VERTEX_ATTRIBS\t: unavailable (requires GL 2.0)\r\n");
			g_ErrorReport.Write("Vertex uniform components: unavailable (requires GL 2.0)\r\n");
		}

		if ((versionParsed && IsOpenGLVersionAtLeast(major, minor, 4, 1)) || GLEW_ARB_ES2_compatibility)
		{
			glGetIntegerv(GL_MAX_VERTEX_UNIFORM_VECTORS, &value);
			g_ErrorReport.Write("Vertex uniform vectors\t: %d\r\n", value);
		}
		else
			g_ErrorReport.Write("Vertex uniform vectors\t: unavailable\r\n");

		const bool uniformBufferCapability =
			(versionParsed && IsOpenGLVersionAtLeast(major, minor, 3, 1)) || GLEW_ARB_uniform_buffer_object;
		if (uniformBufferCapability)
		{
			glGetIntegerv(GL_MAX_UNIFORM_BLOCK_SIZE, &value);
			g_ErrorReport.Write("GL_MAX_UNIFORM_BLOCK_SIZE: %d\r\n", value);
		}
		else
			g_ErrorReport.Write("GL_MAX_UNIFORM_BLOCK_SIZE: unavailable\r\n");

		const bool textureBufferCapability =
			(versionParsed && IsOpenGLVersionAtLeast(major, minor, 3, 1)) || GLEW_ARB_texture_buffer_object;
		if (textureBufferCapability)
		{
			glGetIntegerv(GL_MAX_TEXTURE_BUFFER_SIZE, &value);
			g_ErrorReport.Write("GL_MAX_TEXTURE_BUFFER_SIZE: %d\r\n", value);
		}
		else
			g_ErrorReport.Write("GL_MAX_TEXTURE_BUFFER_SIZE: unavailable\r\n");

		const bool vaoEntries = glGenVertexArrays != NULL && glBindVertexArray != NULL;
		const bool bufferEntries = glGenBuffers != NULL && glBindBuffer != NULL &&
			glBufferData != NULL && glBufferSubData != NULL;
		const bool attributeEntries = glVertexAttribPointer != NULL &&
			glEnableVertexAttribArray != NULL && glDisableVertexAttribArray != NULL;
		const bool integerAttributeEntry = glVertexAttribIPointer != NULL;
		const bool shaderEntries = glCreateShader != NULL && glShaderSource != NULL &&
			glCompileShader != NULL && glGetShaderiv != NULL && glGetShaderInfoLog != NULL &&
			glDeleteShader != NULL;
		const bool programEntries = glCreateProgram != NULL && glAttachShader != NULL &&
			glLinkProgram != NULL && glGetProgramiv != NULL && glGetProgramInfoLog != NULL &&
			glUseProgram != NULL && glDeleteProgram != NULL;
		const bool uniformEntries = glGetUniformLocation != NULL && glUniform1i != NULL &&
			glUniform4f != NULL && glUniform4fv != NULL && glUniformMatrix4fv != NULL;

		g_ErrorReport.Write("<Active VBO required entry points>\r\n");
		WriteEntryPointDiagnostic("glGenVertexArrays", glGenVertexArrays != NULL);
		WriteEntryPointDiagnostic("glBindVertexArray", glBindVertexArray != NULL);
		WriteEntryPointDiagnostic("glGenBuffers", glGenBuffers != NULL);
		WriteEntryPointDiagnostic("glBindBuffer", glBindBuffer != NULL);
		WriteEntryPointDiagnostic("glBufferData", glBufferData != NULL);
		WriteEntryPointDiagnostic("glBufferSubData", glBufferSubData != NULL);
		WriteEntryPointDiagnostic("glVertexAttribPointer", glVertexAttribPointer != NULL);
		WriteEntryPointDiagnostic("glEnableVertexAttribArray", glEnableVertexAttribArray != NULL);
		WriteEntryPointDiagnostic("glDisableVertexAttribArray", glDisableVertexAttribArray != NULL);
		WriteEntryPointDiagnostic("glVertexAttribIPointer", glVertexAttribIPointer != NULL);
		WriteEntryPointDiagnostic("glCreateShader", glCreateShader != NULL);
		WriteEntryPointDiagnostic("glShaderSource", glShaderSource != NULL);
		WriteEntryPointDiagnostic("glCompileShader", glCompileShader != NULL);
		...
		WriteEntryPointDiagnostic("glDrawElements (GL 1.1)", true);

		const bool vaoSupport = vaoEntries &&
			((versionParsed && IsOpenGLVersionAtLeast(major, minor, 3, 0)) || GLEW_ARB_vertex_array_object);
		const bool integerAttributeSupport = integerAttributeEntry &&
			((versionParsed && IsOpenGLVersionAtLeast(major, minor, 3, 0)) || GLEW_EXT_gpu_shader4);
		const bool shaderProgramSupport = shaderVersion && shaderEntries && programEntries && uniformEntries;
		...
		g_ErrorReport.Write("Integer vertex attribs\t: %s\r\n", integerAttributeSupport ? "yes" : "no");
		g_ErrorReport.Write("Shader/program support\t: %s\r\n", shaderProgramSupport ? "yes" : "no");
...
bool CreateOpenglWindow()
{
	...
	if (glewResult == GLEW_OK)
		g_ErrorReport.Write("> GLEW init done (GL %s)\r\n", (const char*)glGetString(GL_VERSION));
	else
		g_ErrorReport.Write("> GLEW init failed; existing shader initialization call remains unchanged.\r\n");
	gShaderScene.Init();
...
BOOL OpenInitFile()
{
	...
	if (ERROR_SUCCESS == RegCreateKeyEx(HKEY_CURRENT_USER, "SOFTWARE\\Webzen\\Mu2\\Config", 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hKey, &dwDisp))
	{
		...
		// targetIndexFPS registry read removed — FPS locked to 60 in steady_clock
...
{
	// Disabled: SetProcessWorkingSetSize races with nvoglv32.dll, crashing on screenshot.
	return;
}
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int nCmdShow)
{
	...
	// FPS dialog removed — locked to 60 FPS in steady_clock
