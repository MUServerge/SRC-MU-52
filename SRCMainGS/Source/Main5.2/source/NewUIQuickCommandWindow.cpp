...
void SEASON3B::CNewUIQuickCommandWindow::RenderFrame()
{
	...
	RenderImageF(IMAGE_QUICKCOMMAND_BACK, x, y, width, height, 0.f, 0.f, (float)((width) * 4.f), (float)((height) * 4.f));

	y = m_Pos.y;
	SEASON3B::RenderImageF(IMAGE_QUICKCOMMAND_FRAME_UP, m_Pos.x, y, 112.f, 45.f);
	y += 45.f;

	for(int i = 0; i < 3; ++i)
	{
		SEASON3B::RenderImageF(IMAGE_QUICKCOMMAND_FRAME_MIDDLE, m_Pos.x, y, 112.f, 15.f);
		y += 15.f;
	}

	SEASON3B::RenderImageF(IMAGE_QUICKCOMMAND_FRAME_MIDDLE, m_Pos.x, y, 112.f, 5.f);
	y += 5.f;

	SEASON3B::RenderImageF(IMAGE_QUICKCOMMAND_FRAME_DOWN, m_Pos.x, y, 112.f, 45.f);

	y = m_Pos.y + 55.f;

	for(int i = 0; i < 4; ++i)
	{
		SEASON3B::RenderImageF(IMAGE_QUICKCOMMAND_LINE, m_Pos.x + 15.f, y, 82.f, 2.f);
		y += 19.f;
	}
}
...
void SEASON3B::CNewUIQuickCommandWindow::RenderArrow()
{
	...
	SEASON3B::RenderImageF(IMAGE_QUICKCOMMAND_ARROWL, x, y, 6.f, 9.f);
	x = m_Pos.x + 90.f;
	SEASON3B::RenderImageF(IMAGE_QUICKCOMMAND_ARROWR, x, y, 6.f, 9.f);
}
