...
void SEASON3B::CNewUIInventoryCtrl::Render(bool RendOption)
{
	...
	for (y = 0; y < m_nRow; y++)
	{
		for (x = 0; x < m_nColumn; x++)
		{
			...
			if (pItem && pItem->Durability == 254 && g_pMyInventory->IsInvenItem(pItem->Type))
			{
				if (Height == 1)
				{
					RenderImageF(IMAGE_ITEM_SQUARE_FOR_1_BY_1, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 21.f, 21.f, 0.f, 0.f, 84.f, 84.f);
				}
				else if (y != m_nRow - 1 && pItem != FindItem(x, y - 1))
				{
					RenderImageF(IMAGE_ITEM_SQUARE_TOP_RECT, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 21.f, 21.f, 0.f, 0.f, 84.f, 84.f);
				}
				else
				{
					RenderImageF(IMAGE_ITEM_SQUARE_BOTTOM_RECT, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 21.f, 21.f, 0.f, 0.f, 84.f, 84.f);
				}
			}
			else
			{
			...
			if (!gmProtect->checkold_school())
			{
				...
				{
					RenderImageF(IMAGE_ITEM_SQUARE, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 21, 21, 0.f, 0.f, 84.f, 84.f);
				}
			}
			else
			{
				...
				if (m_pdwItemCheckBox[iCurSquareIndex] > 1)
				{
					...
					RenderImage(BITMAP_INVENTORY + 18, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), INVENTORY_SQUARE_WIDTH, INVENTORY_SQUARE_HEIGHT, 0.0, 0.0, 1.0, 1.0, color);
	...
	if ((gmProtect->LookAndFeel != 1 && gmProtect->LookAndFeel != 2) && RendOption)
	{
		...
		RenderImageF(IMAGE_ITEM_TABLE_TOP_RIGHT, m_Pos.x + m_Size.cx - WND_RIGHT_EDGE, m_Pos.y - WND_TOP_EDGE, 14, 14, 0.f, 0.f, 56.f, 56.f);
		RenderImageF(IMAGE_ITEM_TABLE_BOTTOM_LEFT, m_Pos.x - WND_LEFT_EDGE, m_Pos.y + m_Size.cy - WND_BOTTOM_EDGE, 14, 14, 0.f, 0.f, 56.f, 56.f);
		RenderImageF(IMAGE_ITEM_TABLE_BOTTOM_RIGHT, m_Pos.x + m_Size.cx - WND_RIGHT_EDGE, m_Pos.y + m_Size.cy - WND_BOTTOM_EDGE, 14, 14, 0.f, 0.f, 56.f, 56.f);

		x = m_Pos.x - WND_LEFT_EDGE + 14;
		y = m_Pos.x + m_Size.cx - WND_RIGHT_EDGE;
		RenderImageF(IMAGE_ITEM_TABLE_TOP_PIXEL, x, m_Pos.y - WND_TOP_EDGE, (y - x), 14, 0.f, 0.f, (float)(((y - x)) * 4.f), 56.f);
		RenderImageF(IMAGE_ITEM_TABLE_BOTTOM_PIXEL, x, m_Pos.y + m_Size.cy - WND_BOTTOM_EDGE, (y - x), 14, 0.f, 0.f, (float)(((y - x)) * 4.f), 56.f);

		y = m_Pos.y - WND_TOP_EDGE + 14;
		x = m_Pos.y + m_Size.cy - WND_BOTTOM_EDGE;
		RenderImageF(IMAGE_ITEM_TABLE_LEFT_PIXEL, m_Pos.x - WND_LEFT_EDGE, y, 14, x - y, 0.f, 0.f, 56.f, (float)((x - y) * 4.f));
		RenderImageF(IMAGE_ITEM_TABLE_RIGHT_PIXEL, m_Pos.x + m_Size.cx - WND_RIGHT_EDGE, y, 14, x - y, 0.f, 0.f, 56.f, (float)((x - y) * 4.f));
