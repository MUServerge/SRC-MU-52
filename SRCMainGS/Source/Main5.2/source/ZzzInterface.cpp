...
int SelectItem()
{
	...
	{
		const int hoveredItem = GetHoveredItemNameDrop(MouseRenderX, MouseRenderY);
		if (hoveredItem != -1)
		{
			OBJECT* object = &Items[hoveredItem].Object;
			object->LightEnable = false;
			Vector(Luminosity, Luminosity, Luminosity, object->Light);
			return hoveredItem;
		}
	}
...
void RenderOutSides()
{
	...
	{
		// Disabled: the fullscreen Karutan dust overlay is unstable on this client.
	}
...
void RenderPartyHP()
{
	...
	for (size_t i = 0; i < MAX_CHARACTERS_CLIENT; i++)
	{
			...
			o->Kind == KIND_MONSTER) && pCharacter->Dead == 0 && pCharacter != Hero)
		{
			...
			// The enhanced monster name pass owns its health bar so both share one
			// projected anchor and a camera-stable screen-space layout.
			if (pCharacter->GetKind() == KIND_MONSTER && gmProtect->m_RenderCharacterName)
			{
				continue;
			}
			...
			{
				// Preserve the established player/party presentation. The new
				// texture-driven renderer is scoped to Look5 monsters and bosses.
				float fLife = 104.f;
				...
				if (fLife > 0.f)
				{
					jpexs::Renderjpexs_dds(IMAGE_HUD_PLAYER, scale_jpexs(ScreenX, -1080), scale_jpexs(ScreenY, 120), 109, 11, 661, 71, 770, 82);
