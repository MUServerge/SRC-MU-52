...
#endif
			{
				if (temp_image->m_ImgIndex == IMAGE_SKILLBOX)
				{
					RenderImageF(temp_image->m_ImgIndex, temp_image->m_Pos.x, temp_image->m_Pos.y,
						temp_image->output_width, temp_image->output_height, 0.f, 0.f, 128.f, 152.f);
				}
				else
				{
					temp_image->Render();
...
void SEASON3B::CNewUIMacroMain::RenderTable(float x, float y, float width, float height)
{
	...
	RenderImageF(IMAGE_MACRO_MAIN_TABLE_TOP_RIGHT, (x + width - 14), y, 14.0, 14.0, 0.f, 0.f, 56.f, 56.f);
	...
	RenderImageF(IMAGE_MACRO_MAIN_TABLE_TOP_PIXEL, (x + 6), y, (width - 12), 14.0, 0.f, 0.f, (float)(((width - 12)) * 4.f), 56.f);

	RenderImageF(IMAGE_MACRO_MAIN_TABLE_RIGHT_PIXEL, (x + width - 14), (y + 6), 14.0, (height - 14), 0.f, 0.f, 56.f, (float)(((height - 14)) * 4.f));
...
void SEASON3B::CNewUIMacroMain::LoadSkillList(int sType)
{
	...
	for (int i = 0; i < MAX_MAGIC; ++i)
	{
		...
		if (Magic_Icon &&
			(Magic_Icon < 67 || Magic_Icon > 72) &&
			!IsBlockedMacroSkill(Magic_Icon))
		{
...
void SEASON3B::CNewUIMacroMain::AddButtonMap(int imgindex, int x, int y, int sx, int sy, bool overflg, bool isimgwidth, bool bClickEffect, bool MoveTxt, const char* btname, const char* tooltiptext, BYTE Number)
{
	...
	{
		temp_button->ChangeButtonImgState(true, IMAGE_IGS_VIEWDETAIL_BTN, overflg, isimgwidth, bClickEffect, 0.f, 0.f, 208.f, 104.f);
	}
	else
	{
		temp_button->ChangeButtonImgState(true, imgindex, overflg, isimgwidth, bClickEffect);
...
void SEASON3B::CNewUIMacroMain::RenderSkillList()
{
	...
	if (SkillList)
	{
		for (auto it = SkillList->begin(); it != SkillList->end(); ++it)
		{
			...
			{
				RenderImageF(IMAGE_SKILLBOX + 1, info->iX, info->iY, 32.0, 38.0, 0.f, 0.f, 128.f, 152.f);
			}
			else
			{
				RenderImageF(IMAGE_SKILLBOX, info->iX, info->iY, 32.0, 38.0, 0.f, 0.f, 128.f, 152.f);
			}
