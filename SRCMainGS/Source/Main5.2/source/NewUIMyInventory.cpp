...
void SEASON3B::CNewUIMyInventory::SetPos(int x, int y)
{
	...
	SEASON3B::LoadPos(&m_BtnJewel, 2, RenderPosX, RenderPosY);
	...
	if (gmProtect->checkold_school())
	{
		...
		SetButtonPosition(&m_BtnJewel, m_Pos.x, m_Pos.y, 2);
		if (GMProtect->IsInvExt())
		{
...
void SEASON3B::CNewUIMyInventory::SetEquipmentInfo(EQUIPMENT_ITEM* pEquipment, int index, int imageindex, int x, int y, int width, int height)
{
	...
	pEquipment->su = pEquipment->sv = pEquipment->uw = pEquipment->vh = 0.f;
}

void SEASON3B::CNewUIMyInventory::SetEquipmentInfo(EQUIPMENT_ITEM* pEquipment, int index, int imageindex, int x, int y, int width, int height, float su, float sv, float uw, float vh)
{
	SetEquipmentInfo(pEquipment, index, imageindex, x, y, width, height);
	pEquipment->su = su;
	pEquipment->sv = sv;
	pEquipment->uw = uw;
	pEquipment->vh = vh;
...
void SEASON3B::CNewUIMyInventory::SetEquipmentSlotInfo()
{
	...
	if (gmProtect->shutdown_item_flag)
	{
		...
		SetEquipmentInfo(&m_Equipment[EQUIPMENT_ARMOR], EQUIPMENT_ARMOR, IMAGE_INVENTORY_ITEM_ARMOR, m_Pos.x + 82, m_Pos.y + 89, 40, 57, 0.f, 0.f, 160.f, 228.f);
		...
		SetEquipmentInfo(&m_Equipment[EQUIPMENT_AMULET], EQUIPMENT_AMULET, IMAGE_INVENTORY_ITEM_NECKLACE, m_Pos.x + 57, m_Pos.y + 89, 22, 22, 0.f, 0.f, 112.f, 112.f);
	...
	else
	{
		...
		SetEquipmentInfo(&m_Equipment[EQUIPMENT_ARMOR], EQUIPMENT_ARMOR, IMAGE_INVENTORY_ITEM_ARMOR, m_Pos.x + 75, m_Pos.y + 87, 46, 66, 0.f, 0.f, 184.f, 264.f);
		...
		SetEquipmentInfo(&m_Equipment[EQUIPMENT_AMULET], EQUIPMENT_AMULET, IMAGE_INVENTORY_ITEM_NECKLACE, m_Pos.x + 54, m_Pos.y + 87, 28, 28, 0.f, 0.f, 112.f, 112.f);
	...
	if (gmProtect->shutdown_item_earring)
	{
		...
		SetEquipmentInfo(&m_Equipment[EQUIPMENT_ERING_L], EQUIPMENT_ERING_L, IMAGE_ITEM_EARRING, pEquipement->x, _RenderFrameY, pEquipement->width, pEquipement->height, 0.f, 0.f, 112.f, 112.f);

		pEquipement = &m_Equipment[EQUIPMENT_RING_RIGHT];

		SetEquipmentInfo(&m_Equipment[EQUIPMENT_ERING_R], EQUIPMENT_ERING_R, IMAGE_ITEM_EARRING, pEquipement->x, _RenderFrameY, pEquipement->width, pEquipement->height, 0.f, 0.f, 112.f, 112.f);
	}
#elif MAIN_UPDATE > 303
	if (gmProtect->checkold_school())
	{
		...
		SetEquipmentInfo(&m_Equipment[EQUIPMENT_AMULET], EQUIPMENT_AMULET, IMAGE_INVENTORY_ITEM_NECKLACE, m_Pos.x + 56, m_Pos.y + 87, 24, 24, 0.f, 0.f, 112.f, 112.f);
...
void SEASON3B::CNewUIMyInventory::SetButtonInfo()
{
	...
	SEASON3B::Loadbutton(&m_BtnJewel, 2, m_Pos.x, m_Pos.y, 20, 20, BITMAP_btn_openshop, false, false, false, 0.f, 0.f, 69.f, 72.f);
	...
	if (gmProtect->checkold_school())
	{
		...
		if (gmProtect->WindowsJewelInventory)
		{
			SetButtonInterface(&m_BtnJewel, m_Pos.x, m_Pos.y, 2);
		}
	...
	else
	{
		...
		m_BtnExit.ChangeButtonImgState(true, IMAGE_INVENTORY_EXIT_BTN, false, false, false, 0.f, 0.f, 144.f, 116.f);
		m_BtnExit.ChangeButtonInfo(RenderPosX, RenderPosY, 36, 29); RenderPosX += 36;
		m_BtnExit.ChangeToolTipText(GlobalText[225], true);

		m_BtnRepair.ChangeButtonImgState(true, IMAGE_INVENTORY_REPAIR_BTN, false, false, false, 0.f, 0.f, 144.f, 116.f);
		m_BtnRepair.ChangeButtonInfo(RenderPosX, RenderPosY, 36, 29); RenderPosX += 36;
		m_BtnRepair.ChangeToolTipText(GlobalText[233], true);

		if (gmProtect->WindowsJewelInventory)
		{
			m_BtnJewel.ChangeButtonImgState(true, IMAGE_INVENTORY_MYSHOP_OPEN_BTN, false, false, false, 0.f, 0.f, 144.f, 116.f);
		...
		if (GMProtect->IsInvExt())
		{
			m_BtnExpansion.ChangeButtonImgState(true, IMAGE_INVENTORY_EXPANSION_BTN, false, false, false, 0.f, 0.f, 144.f, 116.f);
...
void SEASON3B::CNewUIMyInventory::RenderEquippedItem()
{
	...
	for (int i : EquipmentInven)
	{
		...
		if (info->uw > 0.f)
		{
			RenderImageF(info->dwBgImage, info->x, info->y, info->width, info->height, info->su, info->sv, info->uw, info->vh);
		}
		else
		{
#if MAIN_UPDATE > 603
...
void SEASON3B::CNewUIMyInventory::RenderInventoryDetails()
{
	...
	if (gmProtect->checkold_school())
	{
		...
		RenderImageF(IMAGE_INVENTORY_MONEY, m_Pos.x + 11, m_Pos.y + 364, 170.f, 26.f, 0.f, 0.f, 680.f, 104.f);
