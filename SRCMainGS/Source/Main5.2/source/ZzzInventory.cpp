...
void RenderTipTextList(const int sx, const int sy, int TextNum, int Tab, int iSort, int iRenderPoint, BOOL bUseBG, bool bRender3d, bool bUseHudBorder)
{
	float fscale = 0.0;
	...
	if (bUseBG == TRUE && TextNum > 0)
	{
		for (int i = 0; i < 2; i++)
		{
			...
			if (bUseHudBorder)
			{
				HudTooltip::RenderBoxScreen((float)pos[i], fsy[i] - 4.f, fWidth[i], fHeight[i] + 8.f);
				EnableAlphaTest(true);
			}
			else if (gmProtect->shutdown_tooltipold)
...
void RenderSkillInfo(int sx, int sy, int Type, int SkillNum, int iRenderPoint /*= STRP_NONE*/)
{
	...
	if (CharacterAttribute->Skill[Type] == AT_SKILL_INFINITY_ARROW)
	{
		...
		TextListColor[1] = TEXT_COLOR_BLUE; TextBold[1] = false;
		sprintf(TextList[2], "\n");
...
namespace
{
	const float ITEM_NAME_DROP_PADDING = 2.f;

	struct ItemNameDropLayout
	{
		float Left;
		float Top;
		float Width;
	...
	bool BuildItemNameDropLayout(const ITEM_t* item, const OBJECT* object, ItemNameDropLayout* layout)
	{
		if (item == NULL || object == NULL || layout == NULL || item->Name[0] == '\0')
			return false;

		SIZE textSize = { 0, 0 };
		const BOOL measured = g_pMultiLanguage->_GetTextExtentPoint32(
			g_pRenderText->GetFontDC(), item->Name, strlen(item->Name), &textSize);

		if (!measured)
			return false;

		layout->Width = textSize.cx + ITEM_NAME_DROP_PADDING * 2.f;
		layout->Height = textSize.cy + ITEM_NAME_DROP_PADDING * 2.f;
		layout->CenterX = (float)object->ScreenX;
		layout->CenterY = (float)object->ScreenY - 15.f;
		layout->Left = layout->CenterX - layout->Width * 0.5f;
		layout->Top = layout->CenterY - layout->Height * 0.5f;
		return true;
	}
}

int GetHoveredItemNameDrop(float mouseX, float mouseY)
{
	int hoveredItem = -1;
	float nearestDistance = 3.402823466e+38F;
	const HFONT previousFont = g_pRenderText->GetFont();
	g_pRenderText->SetFont(g_hFont);

	for (int i = 0; i < MAX_ITEMS; ++i)
	{
		OBJECT* object = &Items[i].Object;
		if (!object->Live || !object->Visible)
			continue;

		ItemNameDropLayout layout;
		if (!BuildItemNameDropLayout(&Items[i], object, &layout))
			continue;

		if (mouseX < layout.Left || mouseX >= layout.Left + layout.Width
			|| mouseY < layout.Top || mouseY >= layout.Top + layout.Height)
		{
			continue;
		}

		const float dx = mouseX - layout.CenterX;
		const float dy = mouseY - layout.CenterY;
	...
	g_pRenderText->SetFont(previousFont);
	return hoveredItem;
}
...
void RenderItemName(ITEM_t* item, int Number, OBJECT* pObj, int ItemLevel, int ItemOption, int ItemExtOption, bool Sort)
{
	...
	{
		const HFONT previousFont = g_pRenderText->GetFont();
		const DWORD previousTextColor = g_pRenderText->GetTextColor();
		const DWORD previousBgColor = g_pRenderText->GetBgColor();
		g_pRenderText->SetFont(g_hFont);

		ItemNameDropLayout layout;
		if (!BuildItemNameDropLayout(item, pObj, &layout))
		{
			g_pRenderText->SetFont(previousFont);
			g_pRenderText->SetTextColor(previousTextColor);
			g_pRenderText->SetBgColor(previousBgColor);
			glColor4fv(fCurColor);
			return;
		}

		_ScreenX = layout.Left;
		_ScreenY = layout.Top;
		...
		if (gmProtect->LookAndFeel == 5)
		{
			HudTooltip::RenderBoxScreen(_ScreenX, _ScreenY, layout.Width, layout.Height, 0.68f);
			EnableAlphaTest(true);
		}
		else
		{
			jpexs::RenderBoxjpexs_dds(IMAGE_HUD_PLAYER, _ScreenX, _ScreenY, layout.Width, layout.Height);
		}

		g_pRenderText->SetBgColor(0, 0, 0, 0);
		HookColorFont(item->RenderColor);

		g_pRenderText->RenderTextClipped(layout.CenterX, _ScreenY, item->Name, 0, layout.Height, RT3_WRITE_CENTER, NULL);
		g_pRenderText->SetFont(previousFont);
		g_pRenderText->SetTextColor(previousTextColor);
		g_pRenderText->SetBgColor(previousBgColor);
...
{
	SEASON3B::RenderImageF(BITMAP_INVENTORY + 18, x - 1, y - 1, (float)SizeX + 2, (float)SizeY + 2);
