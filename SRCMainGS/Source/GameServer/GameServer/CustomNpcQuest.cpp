...
namespace
{
	bool IsQuestItemMessageFormatSafe(const char* format)
	{
		if(format == 0)
		{
			return false;
		...
		const char* quest = strstr(format,"%d");
		const char* item = strstr(format,"%s");
		if(quest == 0 || item == 0 || quest >= item)
		{
			return false;
		}
		...
		return (conversionCount == 2);
	}

	void BuildQuestItemNotice(char* output,size_t outputSize,int quest,const char* itemText)
	{
		const char* format = gMessage.GlobalText(698);
		if(IsQuestItemMessageFormatSafe(format) != false)
		{
			_snprintf_s(output,outputSize,_TRUNCATE,format,quest,itemText);
		}
		else
		{
			_snprintf_s(output,outputSize,_TRUNCATE,"[QUEST %d] Required item: %s!",quest,itemText);
		}
	}

	bool IsQuestMonsterMessageFormatSafe(const char* format)
	{
		if(format == 0)
		{
			return false;
		...
		const char* firstNumber = strstr(format,"%d");
		const char* secondNumber = ((firstNumber != 0) ? strstr(firstNumber+2,"%d") : 0);
		const char* monster = ((secondNumber != 0) ? strstr(secondNumber+2,"%s") : 0);
		const char* lastNumber = ((monster != 0) ? strstr(monster+2,"%d") : 0);
		if(firstNumber == 0 || secondNumber == 0 || monster == 0 || lastNumber == 0)
		{
			return false;
		}
		...
		return (conversionCount == 4);
	}

	void BuildQuestMonsterNotice(char* output,size_t outputSize,int quest,int quantity,const char* monsterName,int remaining)
	{
		const char* format = gMessage.GlobalText(699);
		if(IsQuestMonsterMessageFormatSafe(format) != false)
		{
			_snprintf_s(output,outputSize,_TRUNCATE,format,quest,quantity,monsterName,remaining);
		}
		else
		{
			_snprintf_s(output,outputSize,_TRUNCATE,"[QUEST %d] Kill %d %s (Remaining: %d)!",quest,quantity,monsterName,remaining);
		}
	}
}
//////////////////////////////////////////////////////////////////////
...
void CCustomNpcQuest::Load(char* path) // OK
{
	...
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
			...
			this->GCCustomNpcQuestHudSend(&gObj[n]); // live reload -> clear stale tracker HUDs
	}
...
bool CCustomNpcQuest::CheckNpcQuest(LPOBJ lpObj,LPOBJ lpNpc) // OK
{
	...
	if(lpObj == 0 || lpNpc == 0 || OBJECT_RANGE(lpObj->Index) == 0 || OBJECT_RANGE(lpNpc->Index) == 0 || gServerInfo.m_CustomNpcQuestSwitch == 0)
	{
		return 0;
	}
...
bool CCustomNpcQuest::CheckCharacter(LPOBJ lpObj,int quest,int Npc) // OK
{
	for(std::vector<CUSTOM_QUEST_CHARACTER>::iterator it=this->m_CustomNpcQuestCharacterInfo.begin();it != this->m_CustomNpcQuestCharacterInfo.end();it++)
	{
		...
		if (it->Level > 0 && lpObj->GetLevel() < it->Level)
		{
	...
	for(std::vector<CUSTOM_QUEST_ITEM>::iterator it=this->m_CustomNpcQuestItemInfo.begin();it != this->m_CustomNpcQuestItemInfo.end();it++)
	{
		...
		if (it->Category >= 0 && it->ItemIndex >= 0)
		{
			...
			if (Qtd < it->Quantity)
			{
				char text[128] = {0};
				char level[16] = {0};
				_snprintf_s(level,sizeof(level),_TRUNCATE,"+%d",it->Level);

				const char* itemName = gItemManager.GetItemName(GET_ITEM(it->Category,it->ItemIndex));
				if(itemName == 0)
				{
					itemName = "Unknown item";
				}

				_snprintf_s(text,sizeof(text),_TRUNCATE,"%d %s %s %s %s",it->Quantity,itemName,(it->Level >= 0) ? level : "",(it->Luck >= 0) ? (it->Luck == 1) ? "+Luck" : "-No Luck" : "",(it->Skill >= 0) ? (it->Skill == 1) ? "+Skill" : "-No Skill" : "");

				char notice[256] = {0};
				BuildQuestItemNotice(notice,sizeof(notice),quest,text);
				gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"%s",notice);

				if (Npc >= 0)
				{
					GCChatTargetNewSend(lpObj,Npc,"%s",notice);
				}
				return 0;
			}
		}
...
CUSTOM_QUEST_MONSTER* CCustomNpcQuest::FindMonsterInfo(int quest) // OK
{
	for(std::vector<CUSTOM_QUEST_MONSTER>::iterator it=this->m_CustomNpcQuestMonsterInfo.begin();it != this->m_CustomNpcQuestMonsterInfo.end();it++)
	{
		if(it->Index > 0 && it->Index == quest && it->MonsterIndex >= 0)
		{
			return &(*it);
		}
	}
...
void CCustomNpcQuest::GCCustomNpcQuestHudSend(LPOBJ lpObj) // OK
{
#if (GAMESERVER_CLIENTE_UPDATE >= 13)

	if(lpObj == 0 || OBJECT_RANGE(lpObj->Index) == 0 || gServerInfo.m_CustomNpcQuestSwitch == 0)
	{
		return;
	}
	...
	PMSG_CUSTOMNPCQUEST_HUD_SEND pMsg = {0};

	pMsg.header.set(0xF3,0xEF,sizeof(pMsg));
	pMsg.map = 0xFF;

	CUSTOM_QUEST_MONSTER* lpInfo = ((lpObj->CustomNpcQuest >= 0) ? this->FindMonsterInfo(lpObj->CustomNpcQuest) : 0);

	if(lpInfo != 0 && lpObj->CustomNpcQuestMonsterIndex >= 0 && lpObj->CustomNpcQuestMonsterQtd >= 0)
	{
		pMsg.state = CUSTOMNPCQUEST_HUD_STATE_ACTIVE;
		pMsg.quest = (WORD)lpObj->CustomNpcQuest;
		pMsg.monsterClass = (WORD)lpObj->CustomNpcQuestMonsterIndex;
		for(std::vector<QUESTNPC_TYPE_INFO>::const_iterator it=this->m_CustomNpcQuestNpcTypeInfo.begin();it != this->m_CustomNpcQuestNpcTypeInfo.end();it++)
		{
			if(it->Index == lpObj->CustomNpcQuest && it->Map >= 0 && it->Map < MAX_MAP)
			{
				pMsg.map = (BYTE)it->Map;
				break;
			}
		}
		pMsg.total = (DWORD)lpInfo->Quantity;
		pMsg.killed = ((lpObj->CustomNpcQuestMonsterQtd < lpInfo->Quantity) ? (DWORD)(lpInfo->Quantity-lpObj->CustomNpcQuestMonsterQtd) : 0);

		const char* monsterName = gMonsterManager.GetMonsterName(lpObj->CustomNpcQuestMonsterIndex);
		if(monsterName != 0)
		{
			strncpy(pMsg.monster,monsterName,(sizeof(pMsg.monster)-1));
		}
	}
	else
	{
		pMsg.state = CUSTOMNPCQUEST_HUD_STATE_NONE;
	}
...
void CCustomNpcQuest::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	if(lpObj == 0 || lpTarget == 0 || OBJECT_RANGE(lpObj->Index) == 0 || OBJECT_RANGE(lpTarget->Index) == 0)
	{
		return;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GlobalText(854),lpObj->CustomNpcQuestMonsterQtd);
	}

	this->GCCustomNpcQuestHudSend(lpObj);
}

bool CCustomNpcQuest::CheckMonster(LPOBJ lpObj,int quest,int Npc,int Count) // OK
{
	if(lpObj == 0 || OBJECT_RANGE(lpObj->Index) == 0 || quest <= 0 || Count < 0)
	{
		return 0;
	}
	...
	for(std::vector<CUSTOM_QUEST_MONSTER>::iterator it=this->m_CustomNpcQuestMonsterInfo.begin();it != this->m_CustomNpcQuestMonsterInfo.end();it++)
	{
		...
		if (it->MonsterIndex >= 0 && lpObj->CustomNpcQuestMonsterQtd != 0)
		{
			...
			const char* monsterName = gMonsterManager.GetMonsterName(it->MonsterIndex);
			if(monsterName == 0)
			{
			...
			char notice[256] = {0};
			BuildQuestMonsterNotice(notice,sizeof(notice),quest,it->Quantity,monsterName,lpObj->CustomNpcQuestMonsterQtd);
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"%s",notice);
...
{
	if(OBJECT_RANGE(aIndex) == 0 || OBJECT_RANGE(NpcIndex) == 0 || gObjIsConnectedGP(aIndex) == 0 || gObjIsConnected(NpcIndex) == 0 || Quest <= 0)
	{
		return;
	}
	...
	SDHP_CUSTOMNPCQUEST_SEND pMsg = {0};
...
void CCustomNpcQuest::DGCustomNpcQuestRecv(SDHP_CUSTOMNPCQUEST_RECV* lpMsg) // OK
{
	...
	if(lpMsg == 0 || gServerInfo.m_CustomNpcQuestSwitch == 0)
	{
		return;
	}

	if(OBJECT_RANGE(lpMsg->index) == 0 || OBJECT_RANGE(lpMsg->indexnpc) == 0 || gObjIsConnectedGP(lpMsg->index) == 0 || gObjIsConnected(lpMsg->indexnpc) == 0)
	{
		return;
	}

	LPOBJ lpObj = &gObj[lpMsg->index];
	LPOBJ lpNpc = &gObj[lpMsg->indexnpc];

	bool validNpc = false;
	for(std::vector<QUESTNPC_TYPE_INFO>::const_iterator it=this->m_CustomNpcQuestNpcTypeInfo.begin();it != this->m_CustomNpcQuestNpcTypeInfo.end();it++)
	{
		if(it->Index == lpMsg->quest && it->MonsterClass == lpNpc->Class && it->Map == lpNpc->Map && it->X == lpNpc->X && it->Y == lpNpc->Y)
		{
			validNpc = true;
			break;
		}
	...
	if (this->CheckQuest(lpObj,quest,lpMsg->indexnpc,lpMsg->questcount) == 0)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GlobalText(860));

			if (lpMsg->indexnpc >= 0)
			{
				GCChatTargetNewSend(lpObj,lpMsg->indexnpc,gMessage.GlobalText(860));
			}
		}
		else 
		{
	...
	SDHP_CUSTOMNPCQUEST_SAVE_SEND pMsg = {0};
	...
	this->GCCustomNpcQuestHudSend(lpObj); // quest done -> client hides the tracker HUD

	//Log
	LogAdd(LOG_EVENT,"[CustomNpcQuest][%s][%s] - (NpcQuest %d: Completed)",lpObj->Account,lpObj->Name,quest);
...
{
	if(OBJECT_RANGE(aIndex) == 0 || gObjIsConnectedGP(aIndex) == 0 || Quest <= 0 || Count < 0)
	{
		return;
	}
	...
	SDHP_CUSTOMNPCQUESTMONSTERSAVE_SEND pMsg = {0};
