...
void EncoderText()
{
	...
	// Resolve the output folder from PathExport (same as EncoderEnviron/EncoderInformation)
	// instead of a hardcoded "..\Client_2" path, so the encoded tooltip lands in the real
	// client folder regardless of its name. The directories are created defensively so the
	// write never fails silently when Data\Local does not exist yet.
	...
	std::string basePath = tempPath.empty() ? std::string("Release") : WStringToUTF8(tempPath);
	std::string dataPath = basePath + "\\Data";
	std::string localPath = dataPath + "\\Local";
	std::string outPath = localPath + "\\CBTextInfo.bin";

	CreateDirectory(basePath.c_str(), 0);
	CreateDirectory(dataPath.c_str(), 0);
	CreateDirectory(localPath.c_str(), 0);

	HANDLE file = CreateFile(outPath.c_str(), GENERIC_WRITE, FILE_SHARE_READ, 0, CREATE_ALWAYS, FILE_ATTRIBUTE_ARCHIVE, 0);

	if (file == INVALID_HANDLE_VALUE)
	{
		printf("CBTextInfo.bin: cannot open '%s' \n", outPath.c_str());
		return;
	}
...
void SaveMainFileInfo(const MAIN_FILE_INFO& mainInfo, const std::string& filePath)
{
	...
	std::vector<CUSTOM_NPC_ICON> CustomNpcIcon;
	...
	runtime_unique_document_file("Local\\CustomNpcIcon.txt", CustomNpcIcon, MAX_CUSTOM_NPC_ICON);
	...
	/*+022*/ writeVector(fp, CustomNpcIcon);	// appended last to keep av-code45.pak backward-compatible
...
void EncoderInformation()
{
	...
	info.ajust_fps_render = GetPrivateProfileInt("Custom", "FpsRenderMax", 60, ".\\MainInfo.ini");
