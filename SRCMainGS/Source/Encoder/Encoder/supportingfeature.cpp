...
void runtime_unique_document_file(std::string filename, std::vector<CUSTOM_NPC_ICON>& item_info, size_t size)
{
	CMemScript* lpMemScript = new CMemScript;
	...
	try
	{
		while (true)
		{
			...
			CUSTOM_NPC_ICON info;

			info.IndexNpc = lpMemScript->GetNumber();

			info.WorldIndex = lpMemScript->GetAsNumber();

			info.OffsetY = lpMemScript->GetAsNumber();

			info.Scale = lpMemScript->GetAsNumber();

			strcpy_s(info.IconFile, lpMemScript->GetAsString());
