...
{
	// Resolution changes recreate the screen-sized font DIB. Release the previous
	// renderer first; overwriting this pointer leaked both its DIB and DC and could
	// exhaust the 32-bit client address space at 2K/4K resolutions.
	Release();
...
void CUIRenderTextOriginal::Release()
{
	if (m_hFontDC != NULL)
	{
		SelectObject(m_hFontDC, GetStockObject(SYSTEM_FONT));
	}

	for (std::map<HFONT, HFONT>::iterator it = m_AntialiasedFonts.begin(); it != m_AntialiasedFonts.end(); ++it)
	{
		DeleteObject(it->second);
	}
	m_AntialiasedFonts.clear();
	m_hFont = NULL;
	if (m_hFontDC != NULL)
	{
		DeleteDC(m_hFontDC);
...
HFONT CUIRenderTextOriginal::GetFont() const
{
	return m_hFont;
}
...
{
	if (m_hFontDC == NULL || hFont == NULL)
		return;

	// A font handle may be reused after a resolution/font-size change. Deselect
	// the cached clone first so GetAntialiasedFont can safely replace it.
	SelectObject(m_hFontDC, GetStockObject(SYSTEM_FONT));
	m_hFont = hFont;
	SelectObject(m_hFontDC, GetAntialiasedFont(hFont));
}

HFONT CUIRenderTextOriginal::GetAntialiasedFont(HFONT hFont)
{
	LOGFONT sourceFont = {};
	if (GetObject(hFont, sizeof(sourceFont), &sourceFont) != sizeof(sourceFont))
		return hFont;

	sourceFont.lfQuality = ANTIALIASED_QUALITY;

	std::map<HFONT, HFONT>::iterator it = m_AntialiasedFonts.find(hFont);
	if (it != m_AntialiasedFonts.end())
	{
		LOGFONT cachedFont = {};
		if (GetObject(it->second, sizeof(cachedFont), &cachedFont) == sizeof(cachedFont)
			&& memcmp(&sourceFont, &cachedFont, sizeof(LOGFONT)) == 0)
		{
			return it->second;
		}

		DeleteObject(it->second);
		m_AntialiasedFonts.erase(it);
	}

	HFONT hAntialiasedFont = CreateFontIndirect(&sourceFont);
	if (hAntialiasedFont == NULL)
		return hFont;

	m_AntialiasedFonts.insert(std::make_pair(hFont, hAntialiasedFont));
	return hAntialiasedFont;
}
...
void CUIRenderTextOriginal::WriteText(int iOffset, int iWidth, int iHeight)
{
	...
	if (m_pFontBuffer == NULL || buff == NULL || iWidth <= 0 || iHeight <= 0)
		return;

	iWidth = min(iWidth, LIMIT_WIDTH);
	iHeight = min(iHeight, LIMIT_HEIGHT);
	const size_t srcBufferSize = (size_t)iPitch * (size_t)FontDCSize.cy;
	const size_t dstBufferSize = pBitmapFont->Buffer.size();
	...
	for (int y = 0; y < iHeight; ++y)
	{
		...
		{
			if (SrcIndex < 0 || (size_t)SrcIndex >= srcBufferSize
				|| DstIndex < 0 || (size_t)DstIndex + sizeof(unsigned int) > dstBufferSize)
			{
#ifdef _DEBUG
				__asm { int 3 };
#endif // _DEBUG
			...
			const unsigned int coverage = *(m_pFontBuffer + SrcIndex);
			const unsigned int textAlpha = (m_dwTextColor >> 24) & 0xFF;
			const unsigned int alpha = (textAlpha * coverage + 127) / 255;
			*((unsigned int*)(buff + DstIndex)) =
				(m_dwTextColor & 0x00FFFFFF) | (alpha << 24);
			SrcIndex += 3;
			DstIndex += 4;
		}
	}
...
void CUITextInputBox::WriteText(int iOffset, int iWidth, int iHeight)
{
	...
	iWidth = min(iWidth, LIMIT_WIDTH);
	iHeight = min(iHeight, LIMIT_HEIGHT);
	const size_t srcBufferSize = (size_t)iPitch * (size_t)RealBoxSize.cy;
	const size_t dstBufferSize = pBitmapFont->Buffer.size();
...
void CUITextInputBox::Render()
{
	...
	m_iCaretBlinkTemp += gsteady_clock->GetFixedUpdateStepCount();
}
...
void CUISlideHelp::Render(BOOL bForceFadeOut)
{
	...
	g_iNoticeInverse += gsteady_clock->GetFixedUpdateStepCount();
}
...
void CRadioButton::RenderImage(GLuint uiImageType, float x, float y, float width, float height, float su, float sv)
{
	...
	const float sourceWidth = (float)pImage->output_width;
	const float sourceHeight = (float)pImage->output_height / 3.0f;
	const int state = (int)(sv / (BTN_HEIGHT + BTN_SPACE));

	SEASON3B::RenderImageF(uiImageType, x, y, width, height,
		0.0f, sourceHeight * state, sourceWidth, sourceHeight);
}
