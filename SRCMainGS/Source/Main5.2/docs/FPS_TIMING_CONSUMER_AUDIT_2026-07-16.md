...
| --- | ---: | ---: | --- |
| `CheckNormalizer` | 11 | 25 | Discrete 25 Hz compatibility pulse |
...
`NewUIMiniMap.cpp`, `UIControls.cpp`, `ZzzEffect.cpp`,
...
1. Migrate deterministic UI/minimap counters with no gameplay authority.
2. Migrate deterministic effect lifetime and blur consumers while preserving
   emitted counts and visible duration.
3. Migrate random pet/boid/Hellas emitters only with explicit stall and RNG rules.
4. Review `standlimit` and `MoveSceneFrame` owners last.
5. Remove a legacy helper only after active calls, callbacks, macros and runtime
