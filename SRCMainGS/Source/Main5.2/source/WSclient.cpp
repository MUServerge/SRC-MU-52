...
void ReceiveCreateMonsterViewport(BYTE* ReceiveBuffer)
{
	...
	for (int i = 0; i < Data->Value; i++)
	{
		...
		if (pCharacter != NULL)
		{
			...
			pCharacter->SetMonsterLevel(Data2->Level);
...
void settings_invasion_manager(BYTE* ReceiveBuffer)
{
	...
	if (gmProtect->InvasionInfoTime)
	{
		char invasionName[sizeof(Data->Name) + 1] = { 0 };
		memcpy(invasionName, Data->Name, sizeof(Data->Name));
		GMInvasionManager->SetInvasion(Data->Index, Data->RemainTime, invasionName);
