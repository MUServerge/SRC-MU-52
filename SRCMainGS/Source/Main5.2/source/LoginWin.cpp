...
#if(CB_DANGKYINGAME)
	//===== "Đăng Ký" (in-game register) button disabled on the login window.
...
#ifdef SAVE_ACCOUNT_SYSTEM
	// Pre-fill the last account saved BY THIS client (stAccountMacro[0]) instead
	// of the stale registry "ID" value, which this client never writes and which
	// can leak in from another MU client sharing the same registry path.
...
#ifdef SAVE_ACCOUNT_SYSTEM
	// Auto-fill the matching password too, when "Save Account" is enabled.
	if (gmProtect->remember_account && m_SaveAccount && !stAccountMacro.empty())
...
#endif
#if(CB_DANGKYINGAME)
	//m_DangKy.SetPosition(nXCoord + 60, nYCoord + 178);	// register button disabled
#endif
}
...
void CLoginWin::Show(bool bShow)
{
	...
	if (gmProtect->remember_account)
	{
		...
		// Refresh the auto-filled account/password every time the window appears,
		// so it always reflects the most recent login (stAccountMacro[0]).
		if (bShow && !stAccountMacro.empty())
		{
			if (m_pIDInputBox)
				m_pIDInputBox->SetText(stAccountMacro[0].NameId.c_str());
			if (m_pPassInputBox)
				m_pPassInputBox->SetText(m_SaveAccount ? stAccountMacro[0].PassID.c_str() : "");
		}
	}
#endif // SAVE_ACCOUNT_SYSTEM
#if(CB_DANGKYINGAME)
	//m_DangKy.Show(bShow);	// register button disabled
#endif
}
...
#if(CB_DANGKYINGAME)
		//else if (m_DangKy.IsClick())	// register button disabled
		//{
...
void CLoginWin::RenderControls()
{
	...
	{
		bool bHasAccount = (m_ID[MAX_ACCOUNT_REG][0] != '\0');
#ifdef SAVE_ACCOUNT_SYSTEM
		// An account pre-filled from this client's saved list also counts.
		if (gmProtect->remember_account && !stAccountMacro.empty())
			bHasAccount = true;
#endif // SAVE_ACCOUNT_SYSTEM
...
void CLoginWin::SaveAccount(std::string NameId, std::string PassID)
{
	if (gmProtect->remember_account)
	{
		// Drop any existing entry for this account, then insert it at the front so
		// stAccountMacro[0] is always the most-recently used login.
		...
		// Keep within the saved-account limit.
		if (stAccountMacro.size() > MAX_ACCOUNT_REG)
			stAccountMacro.resize(MAX_ACCOUNT_REG);
