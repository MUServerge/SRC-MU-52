...
bool SEASON3B::CNewUIMasterSkillTree::Create()
{
	if (g_pNewUIMng)
	{
		...
		this->m_BtnExit.ChangeButtonImgState(1, IMAGE_MASTER_INTERFACE + 5, 0, 0, 0, 0.f, 0.f, 52.f, 56.f);
...
bool SEASON3B::CNewUIMasterSkillTree::Render()
{
	...
	// Backgrounds always render at the window's fixed 640x428 layout size, whatever the
	// skin's texture resolution (the MuDream skins are 4x: 2048+512 x 1712). The split point
	// derives from the textures' own proportions; RenderImageF samples each image whole and
	// scales it (RenderImage would treat w/h as the texture region and crop a hi-res skin).
	float fBackScale = (float)m_Size.cy / (float)pBitMap[0]->output_height;
	float fBackLeftWidth = (float)pBitMap[0]->output_width * fBackScale;
	float fBackRightWidth = (float)pBitMap[1]->output_width * fBackScale;

	RenderImageF(IMAGE_MASTER_INTERFACE, (float)m_Pos.x, (float)m_Pos.y, fBackLeftWidth, (float)m_Size.cy);

	RenderImageF(IMAGE_MASTER_INTERFACE + 1, (float)m_Pos.x + fBackLeftWidth, (float)m_Pos.y, fBackRightWidth, (float)m_Size.cy);
...
void SEASON3B::CNewUIMasterSkillTree::RenderText()
{
	...
	{
		const __int64 iBaseExperience = CharacterAttribute->character_experience(LEVEL_DATA_MASTER, Master_Level_Data.nMLevel);
		const __int64 iCurrentExperience = Master_Level_Data.lMasterLevel_Experince;
		const __int64 iNextExperience = Master_Level_Data.lNext_MasterLevel_Experince;
		const double fBaseExperience = (double)iBaseExperience;
		const double fCurrentExperience = (double)iCurrentExperience;
		const double fNextExperience = (double)iNextExperience;
		double fExperiencePercent = 0.0;
		bool bValidExperience = false;

		if (iCurrentExperience >= iBaseExperience && iCurrentExperience <= iNextExperience)
		{
			if (iNextExperience > iBaseExperience)
			{
				fExperiencePercent = ((fCurrentExperience - fBaseExperience) / (fNextExperience - fBaseExperience)) * 100.0;
			}
			else if (iCurrentExperience == iNextExperience)
			{
				fExperiencePercent = 100.0;
			}

			bValidExperience = true;
		}

		if (bValidExperience == true)
		{
			if (fExperiencePercent < 0.0)
			{
				fExperiencePercent = 0.0;
			}
			else if (fExperiencePercent > 100.0)
			{
				fExperiencePercent = 100.0;
			}

			sprintf_s(Buffer, sizeof(Buffer), GlobalText[3335], fExperiencePercent);
