...
void CMasterSkillTree::Load(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			for (int n = 0; n < MAX_SKILL_TREE_LEVEL; n++)
			{
				info.MainValue[n] = lpMemScript->GetAsFloatNumber();
			}
...
void CMasterSkillTree::SetMasterLevelExperienceTable() // OK
{
	...
	{
		var1 = n + 400; // MAX_CHARACTER_LEVEL
...
void CMasterSkillTree::CalcMasterLevelNextExperience(LPOBJ lpObj) // OK
{
	...
	DWORD MasterLevel = ((lpObj->MasterLevel > MAX_CHARACTER_MASTER_LEVEL) ? MAX_CHARACTER_MASTER_LEVEL : lpObj->MasterLevel);
	QWORD MasterExperience = this->m_MasterLevelExperienceTable[MasterLevel];
	QWORD MasterNextExperience = this->m_MasterLevelExperienceTable[((MasterLevel >= MAX_CHARACTER_MASTER_LEVEL) ? MAX_CHARACTER_MASTER_LEVEL : (MasterLevel + 1))];

	if (lpObj->MasterLevel != MasterLevel)
	{
		LogAdd(LOG_RED, "[CalcMasterLevelNextExperience] Invalid MasterLevel corrected [%s][%s] Level:[%d]->[%d]", lpObj->Account, lpObj->Name, lpObj->MasterLevel, MasterLevel);
		lpObj->MasterLevel = MasterLevel;
	}

	if (lpObj->MasterExperience < MasterExperience || lpObj->MasterExperience > MasterNextExperience)
	{
		LogAdd(LOG_RED, "[CalcMasterLevelNextExperience] Invalid MasterExperience corrected [%s][%s] Level:[%d] Experience:[%I64u] Range:[%I64u-%I64u]", lpObj->Account, lpObj->Name, lpObj->MasterLevel, lpObj->MasterExperience, MasterExperience, MasterNextExperience);
		lpObj->MasterExperience = ((lpObj->MasterExperience < MasterExperience) ? MasterExperience : MasterNextExperience);
	}

	lpObj->MasterNextExperience = MasterNextExperience;
...
void CMasterSkillTree::DGMasterSkillTreeRecv(SDHP_MASTER_SKILL_TREE_RECV* lpMsg) // OK
{
	...
	for (int n = 0; n < MAX_MASTER_SKILL_LIST; n++)
	{
		...
		if (gSkillManager.ConvertSkillByte(&skill, lpMsg->MasterSkill[n]) != 0)
		{
			gSkillManager.AddMasterSkill(lpObj, skill.m_index, skill.m_level);
		}
	}
