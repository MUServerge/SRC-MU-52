...
void CreateWebzenScene()
{
	...
	// MuArchangel2-style boot loading screen (see BITMAP_MUA2_LOADING_BEGIN).
	LoadBitmap("Interface\\KMUA2_Back01.jpg", BITMAP_MUA2_LOADING_BEGIN, GL_LINEAR);
	LoadBitmap("Interface\\KMUA2_Back02.jpg", BITMAP_MUA2_LOADING_BEGIN + 1, GL_LINEAR);
	LoadBitmap("Interface\\KMUA2_Bar.jpg", BITMAP_MUA2_LOADING_BEGIN + 2, GL_LINEAR, GL_REPEAT);
	LoadBitmap("Interface\\KMUA2_BarFrame.tga", BITMAP_MUA2_LOADING_BEGIN + 3, GL_LINEAR);
	LoadBitmap("Interface\\KMUA2_Porcento.tga", BITMAP_MUA2_LOADING_BEGIN + 4, GL_LINEAR);
...
void WebzenScene(HDC hDC)
{
	...
	if (GMConnectHex->GetUpdateVersion() == false)
	{
		...
		for (int i = 0; i <= 4; ++i)
			DeleteBitmap(BITMAP_MUA2_LOADING_BEGIN + i);
...
bool MoveMainCamera()
{
	...
	int iWorld = World;
	const bool usePersistentCameraDistance =
		SceneFlag == MAIN_SCENE
		&& !gMapManager->InBattleCastle()
		&& !gMapManager->IsPKField()
		&& !IsDoppelGanger1()
		&& !IsDoppelGanger2()
		&& !g_Direction.IsDirection()
		&& !CCameraMove::GetInstancePtr()->IsTourMode();

	if (usePersistentCameraDistance)
	{
		// CGMCameraWorld owns gameplay distance even while its input is disabled.
		// F10 therefore freezes the current view instead of resetting zoom.
		CameraDistanceTarget = CurrentCam->distance;
		CameraDistance = CurrentCam->distance;
	}
	...
	{
		// Top view needs enough height for the screen corners as well as the centre.
		// Never reduce the proven 3200 baseline; allow distance-based coverage to
		// raise it when a wider gameplay view requires more terrain.
		CameraViewFar = (CurrentCam->camWidthFar > 3200.f) ? CurrentCam->camWidthFar : 3200.f;
	...
	else
	{
		...
		if (SceneFlag == MAIN_SCENE)
		{
			CameraPosition[2] += CameraFactorPtr->GetPitchOrbitHeight(CameraDistance);
		}
		else
		{
...
bool RenderMainScene()
{
	...
	{
#if MAIN_UPDATE > 303
		if (WindowWidth > 800 || (GMProtect->LookAndFeelType() == 3 || GMProtect->LookAndFeelType() == 4 || GMProtect->LookAndFeelType() == 5))
		{
			Height = GetWindowsY;
		}
		else
...
#if MAIN_UPDATE > 303
	if (gmProtect->ScreenType == 0 && (GMProtect->LookAndFeelType() != 3 && GMProtect->LookAndFeelType() != 4 && GMProtect->LookAndFeelType() != 5))
	{
		Width = GetScreenWidth();
		Height = GetWindowsY - 48;
	}
...
void MainScene(HDC hDC)
{
	...
	{
		CRenderProfilerScope physicsProfilerScope(RP_PHYSICS_RENDER);
	...
	if (Success)
	{
		CRenderProfilerScope swapProfilerScope(RP_SWAP_BUFFERS);
		glFlush();
		SwapBuffers(hDC);
	}
