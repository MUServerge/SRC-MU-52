...
bool CGMProtect::LoadMainFileInfo(MAIN_FILE_INFO& mainInfo, std::string& filePath)
{
	...
	if (fp != nullptr)
	{
		...
		else
		{
			...
			// +022 appended last (matches Encoder writeVector order). Guarded so an
			// older av-code45.pak that predates this vector doesn't read past EOF.
			if (fgetc(fp) != EOF)
			{
				fseek(fp, -1, SEEK_CUR);
				readVector(fp, CustomNpcIcon);
			}
			fclose(fp);
		}
	}
...
CUSTOM_NPC_ICON* CGMProtect::GetNpcIcon(int IndexNpc, int iWorld)
{
	for (size_t i = 0; i < CustomNpcIcon.size(); i++)
	{
		CUSTOM_NPC_ICON* f = &CustomNpcIcon[i];
		...
		if (f->WorldIndex != -1 && f->WorldIndex != iWorld)
			continue;
