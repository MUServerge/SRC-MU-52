...
namespace SEASON3B
{
	...
	typedef struct tagEQUIPMENT_ITEM
	{
		...
		DWORD dwBgImage;
		BOOL CheckRender;
		// Optional source-rect crop (pixels) into dwBgImage, e.g. for a shared sprite
		// sheet. uw==0 means "no crop" - render dwBgImage as its own whole texture
		// (legacy per-slot single-purpose images), same as before this was added.
		float su;
		float sv;
		float uw;
		float vh;
