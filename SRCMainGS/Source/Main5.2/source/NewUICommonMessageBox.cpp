...
#else
	{
		BITMAP_t* pTex = &Bitmaps[m_dwTexType];
		if (pTex->output_width > 0 && pTex->output_height > 0)
		{
			m_fButtonWidth = (float)pTex->output_width;
			m_fButtonHeight = (float)pTex->output_height / 3.0f;
		}
		else
		{
			m_fButtonWidth = width;
...
void SEASON3B::CNewUIMessageBoxButton::Render()
{
	...
	{
		bRender = false;
		SEASON3B::RenderImageF(BITMAP_msgbox_btn_ok, m_x, m_y, m_width, m_height, 0.f, ((float)m_EventState * 120.f), 216.f, 120.f);
	}
	else if (m_dwTexType == CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_CANCEL)
	{
		bRender = false;
		SEASON3B::RenderImageF(BITMAP_msgbox_btn_cancel, m_x, m_y, m_width, m_height, 0.f, ((float)m_EventState * 120.f), 216.f, 120.f);
	}
	else if (m_dwTexType == CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_CLOSE)
	{
		bRender = false;
		SEASON3B::RenderImageF(BITMAP_msgbox_btn_close, m_x, m_y, m_width, m_height, 0.f, ((float)m_EventState * 120.f), 216.f, 120.f);
	}
	else if (m_dwTexType == CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY)
	{
		bRender = false;
		SEASON3B::RenderImageF(BITMAP_msgbox_btn_empty, m_x, m_y, m_width, m_height, 0.f, ((float)m_EventState * 116.f), 432.f, 116.f);
	}
	else if (m_dwTexType == CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_BIG)
	{
		bRender = false;
		SEASON3B::RenderImageF(BITMAP_msgbox_btn_empty_big, m_x, m_y, m_width, m_height, 0.f, ((float)m_EventState * 116.f), 720.f, 116.f);
	}
	else if (m_dwTexType == CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_SMALL)
	{
		bRender = false;
		SEASON3B::RenderImageF(BITMAP_msgbox_btn_empty_small, m_x, m_y, m_width, m_height, 0.f, ((float)m_EventState * 116.f), 256.f, 116.f);
	}
	else if (m_dwTexType == CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_VERY_SMALL
		|| m_dwTexType == BITMAP_btn_empty_very_small)
	{
		bRender = false;
		SEASON3B::RenderImageF(BITMAP_msgbox_btn_empty_very_small, m_x, m_y, m_width, m_height, 0.f, ((float)m_EventState * 92.f), 216.f, 92.f);
	}
#elif MAIN_UPDATE > 303
	if (m_dwTexType == BITMAP_INTERFACE_EXITMENU_BUTTONS)
	{
		// ESC system-menu button (LegendUI\buttons_big.ozt): 3 stacked 252x68 states = normal/hover/down.
		bRender = false;
		SEASON3B::RenderImageF(BITMAP_INTERFACE_EXITMENU_BUTTONS, m_x, m_y, m_width, m_height, 0.f, ((float)m_EventState * 68.f), 252.f, 68.f);
	}
...
void SEASON3B::CTradeCoinMsgBoxLayout::RenderFrame()
{
...
#if MAIN_UPDATE > 303
	SEASON3B::RenderImageF(CNewUIMessageBoxMng::IMAGE_MSGBOX_BACK, x, y + 2.0, 222.0, (GetSize().cy - 8.0), 0.f, 0.f, (float)((222.0) * 4.f), (float)(((GetSize().cy - 8.0)) * 4.f));
	y += 4;
	SEASON3B::RenderImageF(CNewUIMessageBoxMng::IMAGE_MSGBOX_TOP, x, y, 230.0, 67.0, 0.f, 0.f, (float)((230.0) * 4.f), (float)((67.0) * 4.f));
	...
	for (int i = 0; i < 7; ++i)
	{
		SEASON3B::RenderImageF(CNewUIMessageBoxMng::IMAGE_MSGBOX_MIDDLE, x, y, 230.0, height, 0.f, 0.f, (float)((230.0) * 4.f), (float)((height) * 4.f));
		y += height;
	}
#else
	SEASON3B::RenderImageF(CNewUIMessageBoxMng::IMAGE_MSGBOX_BACK, x, y + 2.0, 222.0, (GetSize().cy - 8.0), 0.f, 0.f, (float)((222.0) * 4.f), (float)(((GetSize().cy - 8.0)) * 4.f));
...
#endif
	SEASON3B::RenderImageF(CNewUIMessageBoxMng::IMAGE_MSGBOX_BOTTOM, x, y, 230.0, 50.0, 0.f, 0.f, (float)((230.0) * 4.f), (float)((50.0) * 4.f));
	...
	for (int i = 0; i < 4; ++i)
	{
		SEASON3B::RenderImageF(CNewUICharacterInfoWindow::IMAGE_CHAINFO_TEXTBOX, x + 30, y, 170, 21.0, 0.f, 0.f, 680.f, 84.f);
		y += height;
}
#else
	for (int i = 0; i < 2; ++i)
